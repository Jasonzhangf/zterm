...
- Remote-window unzoomed fullscreen / floating 的单指触控默认属于远端输入：点击发 pointer down/up；拖动在本地识别为 desktop mouse drag，越过阈值立刻发 pointer down，移动中发绝对坐标 pointer move，release 发 pointer up；mouse/trackpad wheel 才保持 pixel scroll。Android app-window touch path 禁止再发延迟的 `gesture/swipe`。即使 `bottomInsetPx` 存在也不得被本地容器 pan 抢走。`bottomChromeInsetPx` 只负责键盘打开时的自动初始上抬，`scale > 1` 后单指拖动才属于本地 zoom pan；不得通过改 terminal renderer、daemon capture 或隐藏本地 pan 补偿远端输入。
...
- remote-window 未放大全屏和浮窗中的单指拖动语义是远端 desktop mouse drag：Android 发送 focus-first pointer down / absolute pointer move / pointer up，不发送 release-only `gesture/swipe`；只有 fullscreen `scale > 1` 时单指拖动属于本地 pan。mouse/trackpad wheel 仍是 pixel scroll。scroll delta 保持 DOM 正向（down/right 为正），daemon `CGEvent` 注入层唯一取反为 macOS wheel 值，禁止 Android 和 daemon 双重取反。
...
- remote-window 输入交付的最低 live gate 是运行中 daemon WebSocket/WebRTC 对受控 AppKit 探针窗口启动 app-window stream，以本地 action 序列 `tap -> drag -> scroll -> key` 生成 focus-first `remote-window-input`，既要看到 `remote-window-input-result accepted=true`，也要从目标进程 stdout 观察到对应 OS 事件。固定入口是 `pnpm --dir android exec tsx scripts/remote-window-live-input-probe.ts`；同时必须跑 `ZTERM_REMOTE_WINDOW_PROBE_MUX=1 ZTERM_REMOTE_WINDOW_PROBE_SESSION=<session> pnpm --dir android exec tsx scripts/remote-window-live-input-probe.ts` 覆盖手机真实 `mux-channel-message` 控制路径。probe 必须创建临时 `.app` bundle、显式创建窗口并写 file-backed event log，并对 catalog 请求显式带 `forceRefresh:true`，避免 daemon 正确返回 60 秒内旧 catalog 而漏掉新探针窗口。clock-skew gate 必须设置 `ZTERM_REMOTE_WINDOW_PROBE_CLIENT_CLOCK_OFFSET_MS=-60000` 跑 raw + mux，证明 daemon 不用 Android/Mac 墙钟差判断 stale。legacy `gesture/swipe` 只能由 daemon owner 单测覆盖兼容性，不能进入 Android touch live gate。没有 raw + mux + clock-skew live gate，只能说单测/构建通过，不能说本地交互验证完成；没有在线 ADB/真机触摸路径，只能说 daemon 协议和 macOS OS-event loopback 已闭环，不能说 installed Android WebView touch delivery 已闭环。
- remote-window 手机侧输入逻辑必须有 page-level gate，不只测 isolated overlay：`TerminalPage.remote-window-overlay.test.tsx` 要渲染真实 `TerminalPage`，打开 picker，选择 interactive app-window，对 `remote-window-video-surface` 重放 touch pointer tap 和 unzoomed touch drag，证明输出是 focus-first pointer down/up 与 focus-first pointer down/move/up，并且 `onTerminalInput` 未被调用。否则 QuickBar/IME 通过不能代表视频 surface touch path 通过。
