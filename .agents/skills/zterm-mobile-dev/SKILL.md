...
- Remote-window 首帧和 stale stream 有独立 gate：ScreenCaptureKit 第一帧早于 daemon `setLocalDescription` / WebRTC media connected 时，只能作为 latest-only pending frame 保存，不能调用 `RTCVideoSource.onFrame`、不能计入 `framesSent`、不能发布 `streaming`；连接 ready 后 flush，stop/cleanup 必须丢弃 pending。daemon 返回 `remote_window_input_stream_missing` 时，Android overlay 必须清 active stream / media stream / stats / pointer state 并显示显式失败，禁止继续显示旧 video surface 接受输入。
