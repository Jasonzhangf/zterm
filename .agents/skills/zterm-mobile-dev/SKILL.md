...
- remote-window 输入交付的最低 live gate 是运行中 daemon WebSocket/WebRTC 对受控 AppKit 探针窗口启动 app-window stream，发送 focus、pointer down/up、scroll、gesture、key down/up，既要看到 `remote-window-input-result accepted=true`，也要从目标进程 stdout 观察到对应 OS 事件。没有这个 gate，只能说单测/构建通过，不能说本地交互验证完成。
