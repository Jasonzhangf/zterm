...
- Remote-window app catalog 是 daemon-owned resident snapshot，不是 tmux/session truth，也不是客户端 cache。daemon runtime 启动时必须 warm 一份完整 app-window+iTerm2 catalog 快照，并由 daemon catalog runtime 自己的 refresh loop 持续刷新；客户端读取只是按 requested source set 过滤并重写 request id 的只读投影，读取本身不得触发 live Swift/iTerm2 枚举，也不得保留会遮蔽 daemon 新快照的客户端 projection cache。`requestRemoteWindowTargetsRuntime` 必须复用当前打开的 SessionContext WebSocket。closed transport 不能被上一份快照伪装为可用。
- Remote-window picker 的 Android overlay 可以持有当前 active session 的上一份 catalog projection，用于 reopen 时先显示旧 rows 并在后台重新读取 daemon 快照；daemon snapshot 才是唯一真源。手动刷新同样只重读 daemon 快照，不能把 picker blank 成大 loading 面板。
