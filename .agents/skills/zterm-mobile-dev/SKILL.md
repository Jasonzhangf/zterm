...
- 新冻结：mobile session group 不能自动从 session 列表补邻居成 wheel；top / center / bottom 必须是显式槽位，未指定时只显示 placeholder。抽屉长按/右键是唯一的槽位分配入口，打开 slot menu 后必须 suppress 下一次 click，避免菜单和 session 激活同时发生。
