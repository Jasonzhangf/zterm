...
- Remote-window APP 多窗口和普通 tmux 多 session 预览必须复用 `src/components/terminal/WindowGroupLayout.tsx` 作为唯一 primary-plus-children 布局 owner。Group 只是 projection 容器；每个 app-window target / Session 都仍是自己的子容器。Remote-window picker 同一 APP 以 `appBundleId + pid` 归组，最大面积窗口默认主窗口；点击子窗口只提升为主窗口，点击当前主窗口才选择该具体 target。Session preview 同理：子 tile 点击只提升 preview 主窗口，只有主 tile 点击才激活真实 shell。禁止在 RemoteWindowOverlay 或 TerminalPreviewGrid 里各自复制一套大窗/小窗布局算法。
