...
- remote-window 输入交付的最低 live gate 是运行中 daemon WebSocket/WebRTC 对受控 AppKit 探针窗口启动 app-window stream，以本地 action 序列 `tap -> swipe/scroll -> key` 生成 focus-first `remote-window-input`，既要看到 `remote-window-input-result accepted=true`，也要从目标进程 stdout 观察到对应 OS 事件。固定入口是 `pnpm --dir android exec tsx scripts/remote-window-live-input-probe.ts`；同时必须跑 `ZTERM_REMOTE_WINDOW_PROBE_MUX=1 ZTERM_REMOTE_WINDOW_PROBE_SESSION=<session> pnpm --dir android exec tsx scripts/remote-window-live-input-probe.ts` 覆盖手机真实 `mux-channel-message` 控制路径。probe 必须创建临时 `.app` bundle、显式创建窗口并写 file-backed event log，并对 catalog 请求显式带 `forceRefresh:true`，避免 daemon 正确返回 60 秒内旧 catalog 而漏掉新探针窗口。clock-skew gate 必须设置 `ZTERM_REMOTE_WINDOW_PROBE_CLIENT_CLOCK_OFFSET_MS=-60000` 跑 raw + mux，证明 daemon 不用 Android/Mac 墙钟差判断 stale。没有 raw + mux + clock-skew live gate，只能说单测/构建通过，不能说本地交互验证完成；没有在线 ADB/真机触摸路径，只能说 daemon 协议和 macOS OS-event loopback 已闭环，不能说 installed Android WebView touch delivery 已闭环。
