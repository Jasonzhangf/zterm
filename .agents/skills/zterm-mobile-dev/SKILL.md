...
- remote-window 手机侧输入逻辑必须有 page-level gate，不只测 isolated overlay：`TerminalPage.remote-window-overlay.test.tsx` 要渲染真实 `TerminalPage`，打开 picker，选择 interactive app-window，对 `remote-window-video-surface` 重放 touch pointer tap 和 unzoomed touch drag，证明输出是 pointer down/up 与一个 release-time `gesture/swipe`（若存在 compatibility focus intent，不可把它当 focus 真源），并且 `onTerminalInput` 未被调用。否则 QuickBar/IME 通过不能代表视频 surface touch path 通过。
