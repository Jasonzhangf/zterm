...
- Remote-window floating resize 必须是 Android overlay projection：至少覆盖左下角和右下角拖拽，按 selected source aspect ratio 等比缩放，右下角扩缩时保持左边稳定并移动右边，左下角扩缩时保持右边稳定。测试不能只证明“有一个 handle”。
