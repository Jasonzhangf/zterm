...
- Remote-window pending/unplayed receiver wallpaper is Android projection only. If the logo source is an opaque RGB asset, normal `<img>` rendering will show a square matte on the dark video surface; use projection blending that hides the asset background and avoid `drop-shadow` because the asset alpha is a full square. The receiver `<video>` must not stay hidden solely until `playing`: Android WebView may reject `play()` or skip `playing` while a muted `MediaStream` is attached, so `loadedmetadata` / `loadeddata` / `canplay` / bounded same-stream fallback may reveal the actual video element. This is a visibility gate only and must not fake daemon capture/WebRTC success.
...
- Remote-window catalog/capture timeout 必须按 owner 对齐：Android catalog watchdog 要长于 daemon app-window catalog timeout，live probe catalog wait 也要更长；Android receiver track timeout 要长于 daemon ScreenCaptureKit startup timeout，因为 daemon 在首帧或显式 capture error 前不会返回 WebRTC answer。修视频打不开时禁止只加 daemon timeout 而不同步 receiver/probe/UI timeout，否则会把 daemon capture 问题误报成客户端无 track 或列表超时。
...
- Remote-window 滚动手感由 client touch/action owner 负责：release-time `gesture/swipe` 和 fullscreen 双指 `scroll` 使用同一套绝对像素语义，按远端可视 source 宽/高计算 1/8、1/4（默认）、1/2、1 屏滚动单位并持久化；反向只翻转 delta sign，不改坐标映射。Zoomed fullscreen 单指始终是本地 pan，双指上下滑才是远端 scroll。修这类问题时必须跑 runtime delta 单测、overlay 双指正/反向测试、page-level physical send 入口测试；禁止到 daemon 做幅度补偿。
...
- client same-revision 覆盖保护只能拦截无请求来源的旧包；foreground/reconnect 后由 renderer/reading 显式发出的 `tail-refresh` / `reading-repair` 如果匹配 pending request 的窗口、`knownRevision` 和 `targetHeadRevision`，即使 revision 相同也必须允许覆盖非 gap 行。否则会出现底部 live 行继续刷、上方旧行永远不刷新的分裂画面。红测必须同时锁住“无 pending request 的 same-revision stale overwrite 仍 drop”和“pending visible refresh same-revision overwrite 可 apply”。
...
- remote-window `<video>` 在真实 `playing` 前不得暴露 WebView/系统原生灰底播放占位；pending/unplayed 状态只显示 ZTERM engraved wallpaper，并在 `MediaStream` attach、metadata、canplay、用户 pointer 进入时重试 `play()`。这只是 projection/播放启动修复，不能当成 ScreenCaptureKit/WebRTC 连接成功证据。
