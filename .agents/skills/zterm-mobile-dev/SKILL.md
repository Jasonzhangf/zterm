...
- Remote-window app catalog 是 daemon-owned resident snapshot，不是 tmux/session truth，也不是客户端 cache。daemon runtime 启动时必须 warm 默认 app-window+iTerm2 catalog，并由 daemon catalog runtime 自己的 refresh loop 持续刷新；client 请求只读该快照，不得触发受信任的 live enumeration，也不得保留会遮蔽 daemon 新快照的客户端 projection cache。`requestRemoteWindowTargetsRuntime` 必须复用当前打开的 SessionContext WebSocket。closed transport 不能被上一份快照伪装为可用。
