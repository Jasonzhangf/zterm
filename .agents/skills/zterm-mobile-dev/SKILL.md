...
- remote-window 输入交付的最低 live gate 是运行中 daemon WebSocket/WebRTC 对受控 AppKit 探针窗口启动 app-window stream，发送 focus、pointer down/move/up、scroll、key down/up，既要看到 `remote-window-input-result accepted=true`，也要从目标进程 stdout 观察到对应 OS 事件。固定入口是 `pnpm --dir android exec tsx scripts/remote-window-live-input-probe.ts`；probe 必须创建临时 `.app` bundle，并对 catalog 请求显式带 `forceRefresh:true`，避免 daemon 正确返回 60 秒内旧 catalog 而漏掉新探针窗口。legacy `gesture/swipe` 只能由 daemon owner 单测覆盖兼容性，不能进入 Android touch live gate。没有这个 gate，只能说单测/构建通过，不能说本地交互验证完成；没有在线 ADB/真机触摸路径，只能说 daemon 协议和 macOS OS-event loopback 已闭环，不能说 installed Android WebView touch delivery 已闭环。
...
- remote-window 真机仍报“不可操作”但无在线 ADB 时，先把 metadata-only 输入诊断投到 `状态`浮窗：CTX 是否 active、context target/route/focus policy、client/session/stream/target id、last source、SEND Y/N、last pointer/key/scroll、坐标、focus/down/move/up/scroll/key/text 计数。Jason 截屏后按分界判断：计数不动是 Android WebView hit-test/overlay path；SEND=N 是 context/sendInput wiring；SEND=Y 且坐标正确但远端不动，才进入 daemon input-result/focus/injection 日志与 live probe。
