...
- Remote-window WebRTC sender 质量闭环必须证明真实 video track。`@roamhq/wrtc` 通过 `addTrack` 创建的 sender 可能没有 encodings，启动阶段若要让 bitrate/FPS 后续可更新，优先用 `addTransceiver(videoTrack, { direction: 'sendonly', sendEncodings: [{ maxBitrate, maxFramerate }] })` seed encodings；没有 transceiver/encodings 时只能显式报告 quality update unsupported，不能伪造成功。Live probe 必须处理 daemon ICE candidate，等待 `ontrack` 或 `getReceivers()` 中的 live video track，并输出 `trackSeen=true`；offer/answer 成功或 `framesSent` 递增不能单独证明 Android/receiver 视频可见。
