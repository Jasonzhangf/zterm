...
- Remote-window input context 只允许发布给已验证支持的 app-window `bring-to-focus + os-event` target。当前 `tmux-input` / `iterm2-api` iTerm pane 路线必须显示只读并禁止发送 pointer/scroll/key/QuickBar input；不要把 daemon 会拒绝的路线伪装成可操作。
...
- Remote-window unzoomed fullscreen / floating 的单指触控默认属于远端输入：点击发 pointer down/up；拖动本地识别成一个 release-time `gesture/swipe`，只在抬手时发送一次起点/终点/delta/duration/velocity，不流式发送 raw touch move，也不排队延迟旧手势；mouse/trackpad wheel 保持 pixel scroll。即使 `bottomInsetPx` 存在也不得被本地容器 pan 抢走。`bottomChromeInsetPx` 只负责键盘打开时的自动初始上抬，`scale > 1` 后单指拖动才属于本地 zoom pan；不得通过改 terminal renderer、daemon capture 或隐藏本地 pan 补偿远端输入。
- Remote-window 滚动手感由 client touch/action owner 负责：release-time `gesture/swipe` 仍在抬手时发一个 bounded action；所有 target-locked 场景的双指上下滑 `scroll` 必须像鼠标滚轮一样按 midpoint 位移连续发送 pixel scroll。1/8、1/4（默认）、1/2、1 屏 preset 只是单次事件最大 cap，不是每次跳转量；默认方向为反向，手动 toggle 只翻转 delta sign，不改坐标映射。Zoomed fullscreen 单指始终是本地 pan，双指上下滑仍是远端 scroll。Pinch zoom 只在 fullscreen 且两根手指都有足够开合距离/比例变化、并且两指沿起始连线轴向移动时成立；即使两指距离变化很大，只要 midpoint 是明确纵向滚动且 pinch 轴向意图不成立，也必须判为 scroll。修这类问题时必须跑 runtime delta 单测、overlay 浮窗双指滚动连续事件、fullscreen 双指正/反向、弱/斜向 pinch 负测、page-level physical send 入口测试；禁止到 daemon 做幅度补偿。
- Remote-window 支持的 app-window 非 `focus` 用户输入必须按“操作瞬间 focus-first”发送：pointer / wheel / key / QuickBar / IME 的每一个真实事件前都要紧邻同 stream/target 的 `focus` intent。stream start、视频 attach、fullscreen 进入、IME 上抬、picker/catalog 刷新、pinch zoom、本地 zoom-pan 都不是远端操作，不得发送 focus。只在 stream start、pointerdown 或 batch 开头 focus 不够；daemon 仍是唯一 AX/Quartz focus+input owner，Android 只能发 intent。unsupported iTerm/tmux read-only 目标仍不得发布 input context 或发送 focus/input。
- Remote-window focus-first 的 daemon helper 必须对“目标 app 已 frontmost 且目标 AX window 已 focused”的重复 focus 快速返回。修 `RW结果 ERR remote_window_input_failed remote window input stale` 这类真机问题时，禁止只跑串行 live probe；必须增加/运行 `ZTERM_REMOTE_WINDOW_PROBE_BURST=1` 的 raw WebSocket、mux-channel、本机 Tailscale mux 黑盒 gate，模拟 Android WebView 连续发送 focus + pointer/key/text 的 burst。串行“发一个等 ACK 一个”的 probe 只能证明协议通，不能证明真机输入不被 helper 队列拖成 stale。
...
- remote-window fullscreen 默认 aspect-fit 完整显示；“填满”语义是向 daemon 发送所选远端目标窗口的 `window-resize` 请求，不是本地 aspect-fill cover/crop。请求后本地绘制和 pointer/input mapping 仍按当前 aspect-fit content rect，直到 daemon 返回新的 capture/manifest truth。禁止为了填满屏幕本地拉伸/裁切冒充 resize，也禁止改 tmux width、terminal mirror 或 renderer。
...
- remote-window 未放大全屏和浮窗中的单指拖动语义是远端滚动手势：Android 本地识别完整 swipe 后在 release 发送一个 focus-first `gesture/swipe`，daemon 把 DOM 正向 delta（down/right 为正）转换为 macOS `CGEvent` scroll；只有 fullscreen `scale > 1` 时单指拖动属于本地 pan。mouse/trackpad wheel 仍是 pixel scroll。scroll/gesture delta 只在 daemon `CGEvent` 注入层取反，禁止 Android 和 daemon 双重取反。
- generic app `os-event` 输入必须有 Accessibility 权限并使用 `bring-to-focus`；只 `NSRunningApplication.activate` 不够，尤其是微信/被遮挡窗口。只有真实用户操作瞬间才允许发 explicit `focus` intent：pointer / wheel / key / QuickBar / IME 事件必须紧邻 focus-first；stream start、视频 attach、fullscreen、IME lift、pinch/local pan 不得抢焦点。iTerm2 pane/read-only target 禁止发送这些 intent。daemon input config 必须带目标 `windowId/title/bounds`，按 `CGWindowList` bounds 匹配 AX window，执行 `AXRaise` / focused window，验证 frontmost + focused 后再发 Quartz pointer/scroll/key event。frontmost 真源用 concrete PID（`System Events` unix id），不要用 bundle id 证明临时/重复 app 实例；`NSRunningApplication.activate()` 返回 true 也不能当成功，必要时用 System Events 按 PID 置前台，并对同 PID `NSRunningApplication(processIdentifier:)` 做短重试后再显式报错。`CGWindowList` top-left 坐标可直接用于 `CGEvent` location，禁止再用 AppKit bottom-left 转换。`no-focus-steal + os-event` 必须显式报错，不能宣称输入成功；验证应使用“目标窗口被 cover window 遮住，未 AXRaise 不动，AXRaise 后真实滚动”的黑盒。
- generic app `os-event` 输入不能每个事件启动一次 `swift -e`；点击、滚动、键盘是连续流，daemon 必须用唯一常驻 Swift helper。交互式 app-window stream start 必须先 warm 并等待 helper ready，且不得发 focus/input；Swift 冷启动不能占用第一个用户操作的 1s realtime deadline。Swift schema 必须匹配 wire union：scroll 没有 `phase`，daemon-only legacy gesture 要求 `phase=end`，pointer/key 才要求 `phase`。黑盒必须通过运行中 daemon WebSocket 启动真实 app-window stream，对受控 AppKit 窗口发送 pointer/scroll/key 并读取目标 stdout，再把 iTerm2 切前台后用 harmless pointer move 验证 WeChat 可被 AXRaise 到前台。
...
- remote-window 输入交付的最低 live gate 是运行中 daemon WebSocket/WebRTC 对受控 AppKit 探针窗口启动 app-window stream，以本地 action 序列 `tap -> swipe/scroll -> key` 生成 focus-first `remote-window-input`，既要看到 `remote-window-input-result accepted=true`，也要从目标进程 stdout/文件日志观察到对应 OS 事件。固定入口是 `pnpm --dir android exec tsx scripts/remote-window-live-input-probe.ts`；同时必须跑 `ZTERM_REMOTE_WINDOW_PROBE_MUX=1 ZTERM_REMOTE_WINDOW_PROBE_SESSION=<session> pnpm --dir android exec tsx scripts/remote-window-live-input-probe.ts` 覆盖手机真实 `mux-channel-message` 控制路径。probe 必须创建临时 `.app` bundle、显式创建窗口并写 file-backed event log，对 catalog 请求显式带 `forceRefresh:true`，并用 `PROBE_READY pid` 精确匹配 catalog target / frontmost PID，避免旧窗口、旧 bundle 或同 bundle 多实例误判。clock-skew gate 必须设置 `ZTERM_REMOTE_WINDOW_PROBE_CLIENT_CLOCK_OFFSET_MS=-60000` 跑 raw + mux，burst gate 必须设置 `ZTERM_REMOTE_WINDOW_PROBE_BURST=1` 跑 raw/mux/Tailscale mux，证明 daemon 不用 Android/Mac 墙钟差判断 stale 且能承受 Android WebView 连续 focus+input。没有 raw + mux + burst + clock-skew live gate，只能说单测/构建通过，不能说本地交互验证完成；没有在线 ADB/真机触摸路径，只能说 daemon 协议和 macOS OS-event loopback 已闭环，不能说 installed Android WebView touch delivery 已闭环。
...
- remote-window 手机侧输入逻辑必须有 page-level gate，不只测 isolated overlay：`TerminalPage.remote-window-overlay.test.tsx` 要渲染真实 `TerminalPage`，打开 picker，选择 interactive app-window，对 `remote-window-video-surface` 重放 touch pointer tap 和 unzoomed touch drag，证明输出是 focus-first pointer down/up 与一个 release-time focus-first `gesture/swipe`，并且 `onTerminalInput` 未被调用。否则 QuickBar/IME 通过不能代表视频 surface touch path 通过。
