...
- 跨 daemon/client/shared/relay/release/observability 的改动，先落 `android/docs/resource-registry.json`、`android/docs/module-registry.json`、`android/docs/edge-registry.json`，再同步 `android/docs/function-map.md`、`android/docs/wiki/mainline-call-map.json`、`android/docs/wiki/mainline-source.md`，最后改 runtime。
- `docs/modules/project-modules.md` 是人读模块图；`module-registry.json` 是模块真源；`edge-registry.json` 是唯一允许跨模块资源边。找不到唯一 `module_id` / `edge_id` 时，不准直接补 runtime patch。
- 连接抽象必须走 `client.daemon_connection`：同一 daemon target 一个 client physical connection；tmux session 是 `client.terminal_channel_mux` 里的 channel，不是 socket owner；heartbeat 是 target-level，不是 session-level。
- `createClientDaemonConnection()` 只能在 transport owner 层创建：`session-context-infra-facade-runtime.ts` 负责共享 read/send owner，`session-context-transport-orchestration-runtime.ts` 只允许创建带 `openSessionTargetTransport` 的 mux open owner。public facade / interaction / lifecycle / message assemblies 只能消费已注入的 `daemonConnection`，禁止各自 new wrapper 或保留 `daemonConnection || raw socket` 双路径。
- `queueSessionTransportOpenIntentRuntime()` 只允许走 mux opener；缺少 `openSessionMuxChannelByIntent` 必须显式失败并删除 pending intent，禁止回退到 `ensureControlTransportForSessionOpen` 或其他 legacy session-ticket socket。
- `getSessionTransportResource()` 只要 session 已有 mux channel，就不得用 `runtime.activeSocket` 冒充有效连接；channel 未 ready 时资源 socket 必须保持空，等待同一 physical transport 上的 mux-ready。
- daemon 侧连接入口走 `daemon.connection_gateway` / `daemon.transport_subscriber`，不得保存 client active tab、foreground/background、viewport、renderer follow/reading、UI projection truth。
...
- macOS Screen Recording 权限、install preflight 和 remote-window capture 必须共用安装态 `zterm-daemon` 这一唯一 TCC/可执行主体：preflight 只运行 `ZTERM_DAEMON_NATIVE --permission-probe`，runtime 只运行同一二进制的 `remote-window-capture` 子命令。权限检查只允许一次 `CGPreflightScreenCaptureAccess()`；禁止 `CGRequestScreenCaptureAccess()`、等待/轮询/重试。禁止 Remote Capture app bundle、独立 raw helper、`ZTERM_DAEMON_CAPTURE_NATIVE`、Node 运行时编译或 screenshot fallback。缺权限/缺 capability 必须立即返回 typed error 并停止；不得切换主体或降级成功。
...
- Remote-window 网络自适应只允许修改 stream-local quality：客户端根据 Network Information 的 `saveData/effectiveType/downlink/rtt` 和 WebRTC `getStats()` 的 RTT、availableIncoming/OutgoingBitrate、framesPerSecond、framesDropped、freezeCount、jitterBufferDelay、qualityLimitationReason 计算不高于用户选择的 effective preset；普通 preset 默认 30 FPS，desktop fullscreen ceiling 60 FPS，stream start 必须把 requested max FPS 传给 ScreenCaptureKit；弱网 stats 要连续命中才降到当前上限 1/4 bitrate 和半 FPS，累计型 drop/freeze/jitter 必须先转成 per-sample delta，稳定窗口后恢复。它不得重建 WebSocket、ScreenCaptureKit、WebRTC receiver 或改变用户记忆的 preset。若实际网络状态变化，质量更新仍走当前 stream 的 quality request。
- Remote-window WebRTC sender 质量闭环必须证明真实 video track。当前 `@roamhq/wrtc` 里 `addTransceiver(videoTrack, { sendEncodings })` 会在真实 Android recvonly offer 下生成 `inactive` answer，ScreenCaptureKit 首帧和 `getReceivers().track.live` 都不能证明 Android `<video>` 会收到 `ontrack`；stream start 必须优先用 `addTrack(videoTrack)` 或其他已验证会生成 `sendonly` 的路径，等 answer/localDescription 落地后再尝试修改已有 sender encodings。没有 encodings 或 `setParameters()` 抛错时只能显式报告 quality update unsupported，不能伪造成功，也不能为码率控制牺牲 video track。Live probe 必须处理 daemon ICE candidate，等待 `ontrack` 事件并输出 `trackSeen=true`；offer/answer、`framesSent`、或只看 `getReceivers()` 都不能单独证明 Android/receiver 视频可见。同一桌面窗口的 ScreenCaptureKit / WebRTC live probe 必须串行运行；并发 raw+mux 或同目标多 stream 容易让 catalog/capture 首帧 timeout，不能作为产品红证据。
...
- Remote-window release-time `gesture/swipe` 仍是 Android 单 action wire contract，但 daemon 注入不能把它变成一次巨大 `CGEvent` wheel delta；必须按起止坐标插值分段 replay，限制单步像素和最大步数，超过 1 秒的排队输入继续丢弃。验证时要看到目标 AppKit log 至少两条 `PROBE_SCROLL`，不能只看 `remote-window-input-result accepted=true`。
...
- remote-window 输入交付的最低 live gate 是运行中 daemon WebSocket/WebRTC 对受控 AppKit 探针窗口启动 app-window stream，以本地 action 序列 `tap -> swipe/scroll -> key` 生成 `remote-window-input`，既要看到 `remote-window-input-result accepted=true`，也要从目标进程 stdout/文件日志观察到对应 OS 事件，并且操作前若 Finder/其他 app 在前台，目标 app 必须先被 daemon 拉到 frontmost。固定入口是 `pnpm --dir android exec tsx scripts/remote-window-live-input-probe.ts`；同时必须跑 `ZTERM_REMOTE_WINDOW_PROBE_MUX=1 ZTERM_REMOTE_WINDOW_PROBE_SESSION=<session> pnpm --dir android exec tsx scripts/remote-window-live-input-probe.ts` 覆盖手机真实 `mux-channel-message` 控制路径。probe 必须创建临时 `.app` bundle、显式创建窗口并写 file-backed event log，对 catalog 请求显式带 `forceRefresh:true`，并用 `PROBE_READY pid` 精确匹配 catalog target / frontmost PID，避免旧窗口、旧 bundle 或同 bundle 多实例误判。clock-skew gate 必须设置 `ZTERM_REMOTE_WINDOW_PROBE_CLIENT_CLOCK_OFFSET_MS=-60000` 跑 raw + mux，burst gate 必须设置 `ZTERM_REMOTE_WINDOW_PROBE_BURST=1` 跑 raw/mux/Tailscale mux，证明 daemon 不用 Android/Mac 墙钟差判断 stale 且能承受 Android WebView 连续 action-only input。没有 raw + mux + burst + clock-skew live gate，只能说单测/构建通过，不能说本地交互验证完成；没有在线 ADB/真机触摸路径，只能说 daemon 协议和 macOS OS-event loopback 已闭环，不能说 installed Android WebView touch delivery 已闭环。
...
- remote-window 手机侧输入逻辑必须有 page-level gate，不只测 isolated overlay：`TerminalPage.remote-window-overlay.test.tsx` 要渲染真实 `TerminalPage`，打开 picker，选择 interactive app-window，对 `remote-window-video-surface` 重放 touch pointer tap 和 unzoomed touch drag，证明输出是一个 `click` action 与一个 release-time `gesture/swipe` action，且没有 client focus prelude，并且 `onTerminalInput` 未被调用。否则 QuickBar/IME 通过不能代表视频 surface touch path 通过。
...
- remote-window installed-phone 手势 / IME L5 不能只看 React `SEND Y`。键盘要用真机 `ImeAnchor.getState()` 证明 `keyboardVisible=true` 且 `keyboardHeight>0`，并留截图；触摸要同时有 CDP/Network `remote-window-input` 与 `remote-window-input-result accepted=true`、受控 AppKit 目标 file-backed log 的 `PHONE_E2E_MOUSE_*` / `PHONE_E2E_SCROLL`、以及重复 tap/swipe 不丢事件。状态栏调试信息只作为定位，不是闭环证据。
- remote-window 真机仍报“不可操作”但无在线 ADB 时，先把 metadata-only 输入诊断投到 `状态`浮窗：CTX 是否 active、context target/route/focus policy、client/session/stream/target id、last source、SEND Y/N、last pointer/key/scroll、坐标、focus/down/move/up/scroll/key/text 计数，以及 daemon `RW结果 ACK/ERR` 与 accepted/error 计数。Jason 截屏后按分界判断：计数不动是 Android WebView hit-test/overlay path；SEND=N 是 context/sendInput wiring；SEND=Y 但无 `RW结果` 是 SessionContext result dispatch / mux channel return path；`RW结果 ERR` 进入显式 daemon error owner；`RW结果 ACK` 且坐标正确但远端不动，才进入真实 target focus/coordinate/AX/Quartz 注入。
- daemon 内部调用 `remoteWindowStreamRuntime.injectInput()` 的 realtime stale 真源只能是 daemon 本地接收/入队时间，不能比较 Android `clientSentAt` 和 Mac `Date.now()`。`clientSentAt` 只保留为跨端 debug metadata；缺失、过去或未来的 client 墙钟不得导致 daemon 拒绝。典型路径是 remote-window image paste 写 macOS clipboard 后注入 Command+V；payload 构造仍放在 `remote-window-stream-daemon` owner helper，禁止 `server.ts` 胶水层手拼输入 payload。daemon stale gate 必须证明 helper 使用 `daemonReceivedAtMs` 且超过 1 秒的 daemon-local 队列项会被拒绝；same-target action-only burst 只能链式刷新相邻 action，不能通过关闭 stale 检查伪装成功。
...
- **动作**:
  ...
  3. Preview tile 复用 `TerminalView` read-only surface：不传 input/resize/viewport callbacks，不 focus DOM，不 copy，不 IME，不改 width mode，不 reconnect。Child/secondary tile 可使用更小的本地 font/row-height 做动态概览，但这只是 renderer projection，不能影响 tmux layout。
  4. Preview 打开时把 selected ids 临时 union 到 live body subscription；关闭、后台、退出后恢复 baseline，只保留 selection preference。
  5. Preview group 复用 `WindowGroupLayout`：child tile 的 title/body tap 只提升为 primary，不激活真实 shell；只有当前 primary tile tap 才走唯一 page owner，把目标 session 投进 focused session-group slot、发一次 explicit active-session switch、退出 preview。进入/退出 preview 不改变 active session。
  6. Preview grid 是 primary-plus-children projection：竖屏 child rail 在上、primary 在下；横屏 child rail 在侧边、primary 在旁边；只创建所需窗口，禁固定铺满 2x3/3x2 空槽，tile titlebar 不显示 `1/2/3/4` 这类视觉 order badge。
  ...
  9. Preview body 允许本地只读上下滚动与 `mirror-fixed` 横向 crop/pan；拖动/滚动不得冒泡成 tile activation、replacement、preview exit、input、resize、viewport 或 width-mode 写。无明显移动的 body tap 可按 tile 语义 promotion/activation。
...
- Mobile default: mux target heartbeat is one timer per physical daemon target, normal interval 30s; target health activity is keyed by target identity, not session id. After the configured consecutive physical-health policy is exhausted, finalize that target socket once as retryable failure and let the existing reconnect owner replace only the physical transport while preserving logical session/channel ids, active-session truth, and buffers. Never add a heartbeat per logical session.
...
- Route-health cache is a circuit breaker, not a long blacklist. Ordinary LAN/Tailscale/WebRTC connectivity failures must use a short cooldown (currently one second) and then become probe-eligible without an Android process restart; authentication failures may keep the full health TTL quarantine. One `TraversalSocket` generation still attempts each candidate at most once.
- Trigger signal: killing/reopening the app immediately restores a reachable Tailscale endpoint while the running process keeps selecting Relay or reports unavailable. Check `route-health-cache` before reconnect/UI code. A process-local five-minute ordinary-failure quarantine is forbidden.
- Physical mux/handshake failure must call the selected transport's `reportFailure()` before target rebuild, clear and close only that exact failed physical generation, preserve logical channel ids, and schedule one target rebuild. Already-closing sockets must not be closed twice.
- Capacitor/DOM network callbacks are generation signals only. They cannot write foreground truth or be scoped through `activeSessionId`; the unique `client.daemon_connection` owner enumerates every current daemon-target physical generation, sends at most one bounded mux probe per target, and dedupes repeated callbacks by exact socket generation.
- `foreground-resume` is a source-discriminated generation-only signal and carries no `connected` / `connectionType` fields. Only Capacitor/window network owners may attach observed connectivity metadata; missing platform truth must never be projected as `connected: true`.
- Foreground entry is also a physical network-generation signal and must reach `client.daemon_connection` before any `hasSessions` / `activeSessionId` / foreground-audit debounce decision. Those session/UI decisions may skip audit or follow-reset, but they cannot suppress probing a retained zero-session daemon-target transport.
- Target socket inbound validation must use `targetKey + exact socket generation`; never bind health/activity acceptance to the logical session that originally opened the socket, because that session may be removed while the physical target transport remains intentionally idle. All target heartbeat and network probe frames use the shared `buildTerminalMuxPing` protocol builder.
- Target probe policy is explicit owner configuration: a callable clock and a positive safe-integer timeout are required constructor inputs. Missing/non-callable clock or missing, zero, negative, fractional, non-finite, or unsafe-integer timeout must throw at construction; never default, round, or clamp them. `buildTerminalMuxPing` likewise accepts only non-negative safe-integer timestamps and must not repair wire payloads. The signal wiring map must include the complete adjacent chain `useOpenTabLifecycleEffects -> useOpenTabRuntime -> AppContent -> SessionProvider -> useSessionProviderFacadeAssemblies -> createSessionPublicFacadeRuntime -> useSessionProviderCoreAssemblies -> createSessionTransportOrchestrationRuntime`; factory-returned closures must be registered by their real factory symbol, never a fabricated static `.probe` symbol.
- Core target-network probe identity/dedupe/failure policy remains TypeScript truth while `terminal.transport_lifecycle.target_network_probe.rust` is `planned`. Do not report the planned Rust path as active until the crate exists, TS/Rust parity gates pass, the bridge is wired, and the old TS policy owner is physically removed.
- A synchronous probe send failure is an explicit `send-failed` result and enters `TerminalTransportError01TargetFailure`; never report it as `started`. The lifecycle/probe/orchestration red tests must run through `test:transport-network-lifecycle` in both Android prebuild and CI before this edge may stay active.
- Live gate for this class: service-scoped daemon restart while the installed APK remains foregrounded; CDP must show the recovered direct endpoint re-probed after cooldown, the same mux channel reopened, and buffer frames resumed. Repeat at least three times. Multiple route/probe WebSocket creations around one target rebuild are a separate connection-gateway failure and must not be hidden by a successful final connection.
- Foreground resume freeze after remote-window fullscreen is not automatically a transport reconnect problem. First check whether `activeBodySubscriptionSuppressedRef` stayed true after background close. `RemoteWindowOverlay` must release body suppression on `appForegroundActive=false` before/while stopping the stream, and focused gates must assert final `onActiveBodySubscriptionSuppressedChange(false)` plus same-socket foreground head/body render update. Do not add App/TerminalPage reconnect fallback for this class.

## File transfer wire frame budget
- Trigger: sync upload crashes even after native `readFileChunk` streaming.
...
## Reliable input resend policy
- Trigger: weak network causes typed input to appear multiple times.
- Rule: a reliable input item already sent is in-flight until ACK/NACK. The client may observe on a cadence, but it may resend only after bounded ACK timeout or target route generation change, and must reuse the same seq.
- Anti-pattern: fixed interval resend before ACK, clearing `sentAt` on every timer tick, or using daemon dedupe to hide client over-send.
- Marker: `reliable input ack timeout route generation resend same seq`

## Light terminal shell contrast
- Trigger: the light terminal appears as a glaring white sheet, its text becomes too dark, or QuickBar labels look flat while the buttons are raised.
- Rule: change the renderer preset background and foreground at the terminal theme owner when the complaint is about terminal body brightness. Use established GitHub Primer neutrals (`#eaeef2` surface, `#57606a` foreground) and keep shell chrome on the matching grouped gray scale. QuickBar labels use a restrained dark-above/light-below engraved text shadow; blue/black buttons keep explicit highlight, inset shade, and outer shadow tokens.
