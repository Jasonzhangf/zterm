...
- session group layout axis 默认按 aspect ratio：窄竖屏（当前阈值 `width / height <= 0.7`）强制 vertical，上下滚；宽竖屏默认 horizontal，但 Settings 可切 vertical；landscape 永远 horizontal。这个设置只影响 layout projection，禁止改写 drawer 固定槽位、session/tab/pane 真相。
