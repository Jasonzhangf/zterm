...
- Remote-window 滚动手感由 client touch/action owner 负责：release-time `gesture/swipe` 和 fullscreen 双指 `scroll` 使用同一套 amplitude tuning，默认 2x，可在 1/1.5/2/3/4x 内调整并持久化；反向只翻转 delta sign，不改坐标映射。Zoomed fullscreen 单指始终是本地 pan，双指上下滑才是远端 scroll。修这类问题时必须跑 runtime delta 单测、overlay 双指正/反向测试、page-level physical send 入口测试；禁止到 daemon 做幅度补偿。
