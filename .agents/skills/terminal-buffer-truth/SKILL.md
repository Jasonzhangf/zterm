...
- 若 wire payload 带 `frameChunkCount > 1`，还必须先审 client buffer owner 是否逐 chunk publish：
  ...
  12. explicit resume 的 lower-revision fresh body 若早于 head 到达，只有完整 authoritative tail 可以建立新 epoch。unchunked body 必须同时满足显式 available bounds 等于 payload bounds，且连续行精确覆盖该 bounds；chunked body 必须先完成 frame assembly 并用完整 frame window 证明覆盖。dense partial window、缺 available metadata 或 malformed frame 只能 stale drop，禁止借此清空 pending frame、repair ledger 或 local buffer
