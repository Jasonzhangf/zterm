...
- 若现场表现为“**底部 prompt/status/input 重复刷到上方**”，黑盒 gate 必须自动比较 source buffer 与 DOM output：source 里 prompt/status/input 各 1 行，DOM 也只能各 1 行；DOM row absolute index 必须唯一且文本必须按 absolute index 精确等于 source；同时断言上一帧 prompt/body 不残留。daemon `top-live` oracle 只能证明 tmux -> daemon source，不证明 Android WebView DOM。
...
- 若现场表现为“**抽屉/分组切换后画面像切到新 session，但输入仍进旧 session 或新画面不刷新**”，优先查 session group center projection 是否早于 runtime active truth。`TerminalPage` 只能先记录 pending activation；必须等 `uiSessionId/interactiveSession` 确认目标 active 后再更新 slot/focus。`TerminalStageShell` 的 group center `active/onInput/onResize/onWidthModeChange` 只能由 `interactiveSession` 决定，禁止因为 center slot 投影就提前给输入 owner。
