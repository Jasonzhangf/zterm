...
- daemon 不得长期保存 UI / renderer width policy；`adaptive-phone` 只能进入唯一 adaptive width lease owner：
  ...
  - tmux 的 `resize-window -x` 会把 window 切成 `window-size=manual`；final release 必须先恢复 baseline 宽度和高度，最后再执行 `set-window-option -u window-size`。若先 unset 再 resize，resize 会再次留下 `manual`，iTerm2 等真实 tmux client 会被冻结在 daemon 最后一帧尺寸，历史/重排表现异常。
...
- `adaptive-phone` 的 tmux side effect 必须单点化：只允许 `terminal-mirror-runtime.ts` 的 adaptive lease owner 执行运行期 `resize-window -x`，以及 final release 的 baseline `resize-window -x <cols> -y <rows>` 和 `set-window-option -u window-size`；禁止 daemon-start、renderer、UI、foreground/background、普通 resize/attach 分支散落执行。
...
- 若 wire payload 带 `frameChunkCount > 1`，还必须先审 client buffer owner 是否逐 chunk publish：
  ...
  9. frame assembly 必须服从 shared protocol 的 span/chunk/retained-byte/lifetime 硬上限；body ingress 写 map 前拦截容量越界，buffer-head cadence 或下一次 chunk 到达时发现到期都必须主动释放 incomplete frame，并只通过 exact-range repair owner 请求一次。禁止把“等后续 buffer-head/下一 chunk/等 session close 清理”当生命周期边界
  ...
  12. explicit resume 的 lower-revision fresh body 若早于 head 到达，只有完整 authoritative tail 可以建立新 epoch。unchunked body 必须同时满足显式 available bounds 等于 payload bounds，且连续行精确覆盖该 bounds；chunked body 必须先完成 frame assembly 并用完整 frame window 证明覆盖。dense partial window、缺 available metadata 或 malformed frame 只能 stale drop，禁止借此清空 pending frame、repair ledger 或 local buffer；staged body-first frame 的 malformed/interleave/resource-limit输入只拒绝本次输入并保留已收 chunks，唯有过期候选走 exact-range repair
