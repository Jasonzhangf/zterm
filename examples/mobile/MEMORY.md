...
- 项目目标：Android 终端应用，Capacitor + @wterm/react
- 手机端定位：纯 client
...
- [2026-04-19] daemon / connection 鉴权真源使用共享 token：server 优先读 `~/.wterm/config.json -> mobile.daemon.authToken`，`WTERM_MOBILE_AUTH_TOKEN` 只作显式 override；client 从 remembered target / host `authToken` 透传到 websocket query
