...
export function resolveDaemonRuntimeConfig(options?: {
  ...
  const daemonConfig = config.mobile?.daemon || {};

  const host =
    asString(env.WTERM_MOBILE_HOST) ||
    asString(env.HOST) ||
    asString(daemonConfig.host) ||
    DEFAULT_DAEMON_HOST;

  const port =
    asPositiveInteger(env.WTERM_MOBILE_PORT) ||
    asPositiveInteger(env.PORT) ||
    asPositiveInteger(daemonConfig.port) ||
    DEFAULT_BRIDGE_PORT;

  const authTokenFromEnv = asString(env.WTERM_MOBILE_AUTH_TOKEN);
  ...
  const terminalCacheLines =
    asPositiveInteger(env.WTERM_MOBILE_TERMINAL_CACHE_LINES) ||
    asPositiveInteger(daemonConfig.terminalCacheLines) ||
    DEFAULT_TERMINAL_CACHE_LINES;

  const sessionName =
    asString(env.WTERM_MOBILE_DAEMON_SESSION) ||
