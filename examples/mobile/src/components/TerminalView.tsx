...
export function TerminalView({
  ...
  const updateFollowOutputFromHost = useCallback((host: HTMLDivElement) => {
    ...
    syncDebugMetrics();
  }, [resolvedLineHeightPx, scrollbackStartIndex, syncDebugMetrics, syncScrollViewportState]);
  ...
  useLayoutEffect(() => {
    ...
    const restoreReadingAnchor = () => {
      ...
      const heightDelta = host.scrollHeight - previous.scrollHeight;
      if (bufferUpdateKind === 'prepend') {
        host.scrollTop = Math.min(maxScrollTop, Math.max(0, previous.scrollTop + heightDelta));
        return;
      }
    ...
    if (activated) {
      if (!scrollLockRef.current && followOutputRef.current) {
        scrollToBottom();
      } else {
        restorePreviousScrollTop();
      }
    ...
    if (previous.bufferRevision !== bufferRevision) {
      const shouldFollowOutput = followOutputRef.current && !scrollLockRef.current;
      if (!shouldFollowOutput || scrollLockRef.current) {
        restoreReadingAnchor();
      } else {
        scrollToBottom();
        followOutputRef.current = true;
      }
    } else if (!scrollLockRef.current && followOutputRef.current) {
      scrollToBottom();
    } else {
      restorePreviousScrollTop();
    }
  ...
  useEffect(() => {
    ...
    syncScrollViewportState(host);
    syncDebugMetrics();
  }, [active, bufferRevision, forceScrollToBottomNonce, resolvedLineHeightPx, scrollbackStartIndex, syncDebugMetrics, syncScrollViewportState]);
    ...
    >
      <div className="term-grid" data-cursor-source={snapshot ? 'remote' : 'hydrated'}>
        ...
        {viewportRows.map((row, rowIndex) => (
          <div key={`vp-${rowIndex}`} className="term-row" style={{ height: resolvedRowHeight || rowHeight, lineHeight: resolvedRowHeight || rowHeight }}>
            {row.length > 0
              ? row.map((cell, cellIndex) => {
                  const isCursor = Boolean(snapshot && cursor?.visible && cursor.row === rowIndex && cursor.col === cellIndex);
                  const content = cell.width === 0 ? '' : cell.char >= 32 ? String.fromCodePoint(cell.char) : ' ';
                  const style = cellStyle(cell, resolvedRowHeight || rowHeight);
                  if (isCursor) {
