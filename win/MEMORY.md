...
- 2026-07-29 iTerm2-style pane UX extension stays in `windows.desktop_shell.workspace`: blank panes can be active chooser targets, session list rows open directly into the active pane, existing tabs expose `Change session` and explicit `Move to Pn`, and source panes keep an empty tab when their last runtime tab is moved. Local white-box/UI tests cover this; packaged Windows target install/live smoke is still required before claiming remote desktop closure.
- `Change session` is a scoped chooser intent, not an immediate replacement with the global form target. Context-menu action stores `{paneId, tabId}`, activates that pane, opens the session list, and only `session-list` row click calls `changeWindowsWorkspaceTabSession`. Marker: `windows change session scoped chooser before replacement`.
- Dismissing or cancelling the scoped chooser must clear the pending replacement intent; otherwise a later unrelated session-list click can replace the originally selected tab. Marker: `windows scoped replacement intent cleared on chooser dismiss`.
...
- [2026-07-29] Windows workspace `Move to Pn` applies only to runtime tabs with a real target; empty placeholder tabs are chooser slots and must not be moved between panes. Windows pane context menus also dismiss on outside pointer down, Escape, and stale tab removal. Marker: `windows empty placeholder tab not movable context menu dismiss`.
- [2026-07-29] Windows workspace scoped replacement applies to both session-list rows and the primary `连接` action. While `pendingSessionReplacement` is set, `连接` must call `changeWindowsWorkspaceTabSession` instead of opening a new tab; split actions remain new-pane/open behavior. Marker: `windows primary connect consumes scoped replacement`.
