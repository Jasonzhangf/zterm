...
fn clearWideArtifactsAroundCursor(self: *Terminal) void {
    const current = self.grid.getCell(self.cursor_row, self.cursor_col);

    if (current.width == cell_mod.WIDTH_DOUBLE and self.cursor_col + 1 < self.cols) {
        self.grid.setCell(self.cursor_row, self.cursor_col + 1, Cell{});
    }

    if (current.width == cell_mod.WIDTH_CONTINUATION and self.cursor_col > 0) {
        const prev_col = self.cursor_col - 1;
        const prev = self.grid.getCell(self.cursor_row, prev_col);
        if (prev.width == cell_mod.WIDTH_DOUBLE) {
            self.grid.setCell(self.cursor_row, prev_col, Cell{});
        }
    }
...
pub const Terminal = struct {
    ...
    fn printChar(self: *Terminal, codepoint: u21) void {
        ...
        const width: u8 = if (isWideCodepoint(codepoint)) cell_mod.WIDTH_DOUBLE else cell_mod.WIDTH_SINGLE;
        clearWideArtifactsAroundCursor(self);

        if (width == cell_mod.WIDTH_DOUBLE and self.cursor_col == self.cols - 1) {
            if (self.auto_wrap) {
                self.cursor_col = 0;
                self.doLinefeed();
            } else {
        ...
        if (width == cell_mod.WIDTH_DOUBLE and self.cursor_col + 1 < self.cols) {
            self.grid.setCell(self.cursor_row, self.cursor_col + 1, Cell{
                .char = ' ',
                .fg = self.current_fg,
                .bg = self.current_bg,
...
test "wide chars occupy two cells and advance remote cursor by two" {
    const testing = @import("std").testing;
    var t = Terminal.init(10, 3);
    t.write("你A");
    try testing.expectEqual(@as(u32, '你'), t.grid.getCell(0, 0).char);
    try testing.expectEqual(cell_mod.WIDTH_DOUBLE, t.grid.getCell(0, 0).width);
...
test "overwriting a wide cell with narrow text clears continuation artifacts" {
    const testing = @import("std").testing;
    var t = Terminal.init(10, 3);
    t.write("你");
    t.write("\rA");
    try testing.expectEqual(@as(u32, 'A'), t.grid.getCell(0, 0).char);
    try testing.expectEqual(cell_mod.WIDTH_SINGLE, t.grid.getCell(0, 0).width);
    try testing.expectEqual(cell_mod.WIDTH_SINGLE, t.grid.getCell(0, 1).width);
    try testing.expectEqual(@as(u32, ' '), t.grid.getCell(0, 1).char);
}
