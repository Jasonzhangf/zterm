...
describe("WasmBridge", () => {
  ...
  describe("writeString / getCell", () => {
    ...
    it("writes multiple characters sequentially", () => {
      ...
      expect(bridge.getCell(0, 0).width).toBe(1);
    });
  ...
  describe("cursor", () => {
    ...
    it("advances cursor by two cells for wide unicode chars", () => {
      bridge.writeString("你");
      const cursor = bridge.getCursor();
      expect(cursor.col).toBe(2);
      expect(bridge.getCell(0, 0).char).toBe("你".codePointAt(0));
