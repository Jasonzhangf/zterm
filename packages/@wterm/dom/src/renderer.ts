...
const WIDTH_CONTINUATION = 0;
const WIDTH_SINGLE = 1;
const WIDTH_DOUBLE = 2;
  ...
  private _buildRowContent(
    ...
    for (let col = 0; col < this.cols; col++) {
      ...
      if (inBounds && cp >= 0x2580 && cp <= 0x259f) {
        ...
        span.style.width = cell.width === WIDTH_DOUBLE ? "2ch" : "1ch";
        if (cell.flags & FLAG_DIM) span.style.opacity = "0.5";
        rowEl.appendChild(span);
      } else {
        const span = document.createElement("span");
        span.className =
          col === cursorCol ? "term-cell term-cursor" : "term-cell";
        const cellWidth = inBounds ? cell.width : WIDTH_SINGLE;

        if (cellWidth === WIDTH_CONTINUATION) {
          span.classList.add("term-cell-continuation");
        ...
        span.style.width = cellWidth === WIDTH_DOUBLE ? "2ch" : "1ch";
        span.textContent = inBounds && cp >= 32 ? String.fromCodePoint(cp) : " ";
        rowEl.appendChild(span);
      }
    }
  }
