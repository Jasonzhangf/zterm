...
function createMockBridge(): WasmBridge {
  return {
    ...
    getRows: vi.fn(() => 24),
    getCols: vi.fn(() => 80),
    getCell: vi.fn(() => ({ char: 0, fg: 256, bg: 256, flags: 0, width: 1 })),
    isDirtyRow: vi.fn(() => true),
    clearDirty: vi.fn(),
    getCursor: vi.fn(() => ({ row: 0, col: 0, visible: true })),
    getScrollbackCount: vi.fn(() => 0),
    getScrollbackCell: vi.fn(() => ({ char: 0, fg: 256, bg: 256, flags: 0, width: 1 })),
...
describe("WTerm", () => {
  ...
  describe("scrollback class toggle", () => {
    ...
    it("preserves viewport anchor when scrollback grows above the current scroll position", async () => {
      ...
      vi.mocked(mockBridge.clearDirty).mockImplementation(() => {
        scrollHeightValue = 140;
      });

      (term as unknown as { _shouldScrollToBottom: boolean })._shouldScrollToBottom = false;
      (term as unknown as { _doRender: () => void })._doRender();
