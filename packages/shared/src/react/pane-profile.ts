...
function buildDesktopSplitProfile(): PanePlatformProfile {
  return {
    ...
    header: {
      outerPadding: (topInsetPx: number) =>
        `${Math.max(0, topInsetPx) + DESKTOP_HEADER_TOUCH_SAFE_OFFSET_PX}px 12px 8px`,
      rowGap: '8px',
      paneGap: '6px',
      backButtonSize: '28px',
      backButtonRadius: '8px',
...
function buildDesktopSingleProfile(): PanePlatformProfile {
  return {
    ...
    header: {
      outerPadding: (topInsetPx: number) =>
        `${Math.max(0, topInsetPx) + DESKTOP_HEADER_TOUCH_SAFE_OFFSET_PX}px 16px 10px`,
      rowGap: '8px',
      paneGap: '6px',
      backButtonSize: '32px',
      backButtonRadius: '8px',
