...
## 2026-07-07 Android session group switch false-active

- 现场问题：抽屉/分组切换后 UI 显示像已经切到新 session，画面也换了，但输入仍打到上一个 session，且新画面不继续刷新。
- 架构映射：属于 `terminal.session_group_layout` / UI shell projection 与 `terminal.workspace_panes` active runtime 边界；禁止改 daemon、buffer manager 或 TerminalView 输入逻辑。
