...
- Read-only evidence: `useSessionContextLifecycle` previously triggered explicit resume only when `appForegroundActive` changed false -> true. `useOpenTabLifecycleEffects` handled `visibilitychange`, document `resume`, and Capacitor `appStateChange`, but foreground signals only updated the boolean and audited remote tmux truth. If Android emitted foreground `resume`/`appStateChange(true)` while the React boolean was already `true` or if the hidden event was missed/coalesced, the active transport owner never received a resume event.
- Root cause: foreground resume was modeled as a boolean edge, but Android foreground restore is an event. The first divergence was the missing event identity between `useOpenTabLifecycleEffects` foreground event observation and `useSessionContextLifecycle` explicit-resume invocation. Unique owner remains SessionContext lifecycle.
