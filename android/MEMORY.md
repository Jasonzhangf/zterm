...
- [2026-07-03] FileTransferSheet 的 Android 本地文件访问必须同时满足三条：缺存储权限时 UI owner 主动调用 `StoragePermissionPlugin.request()`；从 Android 设置页返回后用 focus/visibility 刷新权限并重新列目录；`Directory.ExternalStorage` 调用只传外部存储根下相对路径（如 `Download/zterm`），UI 保持显示 `/storage/emulated/0/...`。本地目录读取失败必须显式投影错误，禁止清空列表后显示成“空目录”。
