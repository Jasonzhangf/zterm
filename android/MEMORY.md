...
- [2026-06-30] UI 入口语义收口：Connections 主页面入口必须保留，但语义是“进入/连接服务器 workspace”，不是 `New Session` / `New Tab` 创建，也不得打开 `new-connection` picker；session/daemon 操作 owner 是 Terminal drawer。drawer host rail 列账号目录 daemon，底部 `New Session` 必须按当前选中机器调用 daemon `tmux-create-session` 创建空白 session 后打开，禁止再跳回通用 new-connection picker。
