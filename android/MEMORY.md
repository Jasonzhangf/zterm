...
- [2026-08-18] Terminal container shrink / black screen: root cause was a layout geometry defect inside `client.terminal_shell`, not a buffer/repair failure. `TerminalStageShell`'s absolute stage wrapper had only `flex: 1`/`minHeight: 0`; it was not itself a column flex container, so PaneStage and `.wterm` lost a stable height when the WebView viewport shrank, leaving the renderer with `clientHeight` near 0 and the visible tail showing as a blank black canvas. A separate page-level `useLayoutEffect` was fabricating `viewportRows/viewportEndIndex` from `sessionBufferStore.rows/endIndex` for non-active panes, which mixed buffer truth into the renderer window and was deleted as part of the same fix. The shell memo comparator also omitted `terminalChromeTopPx`, so a top-inset relayout could be skipped after a previous memoized absolute position.
- Fix scope is strictly `client.terminal_shell`: `TerminalStageShell`'s absolute stage wrapper now `display:flex;flexDirection:column;minHeight:0`, memo comparator includes `terminalChromeTopPx`, and the page-level split effect that wrote viewport demand from buffer rows/endIndex is removed. `TerminalView` remains the sole owner of DOM measurement / `viewportRows` / renderer window; the client sparse buffer and renderer are unchanged.
- Validation gates run before merge: `git diff --check`, `pnpm run type-check` PASS, focused tests `117/117` (`TerminalView`, `TerminalView.dynamic-refresh`, `TerminalPage.multi-pane-decouple`, `TerminalPageStageShell.pane-stage`, `terminal-viewport-metrics`), `test:feature-registry` `95/95`, full `pnpm run build` PASS, Gradle APK build PASS, `adb install -r` over the prior APK preserves user data, and `daemon:mirror:close-loop` keeps 9/9 cases PASS without any daemon-side change.
- Real device evidence on `100.104.163.65:5555` (PLZ110, APK 0.1.3.2666 / versionCode 1100026660, sha256 `9e783a073381161303018799360e6db536a775c123a348ac18dbe85fc377b587`):
  - shrink path (`wm size 720x1600`): `viewport 205x457`, stage `top=89 bottom=292 height=203`, stage child `height=203`, `.wterm` `height=179`, `clientHeight=179`, 15 DOM rows with `data-terminal-index` 14701..14715, no `data-terminal-line-discontinuous`, terminal tail is real text (`gpt-5.5 high · /Volumes/extension/code/dsh-plugins  Goal stalled (/goal resume)`); Pillow on `region (0,200,720,850)` reports non-black fraction `0.2796`, light pixel fraction `0.0161` -> not black. Evidence: `android/evidence/2026-08-18/terminal-container-shrink-black/shrink-after-720x1600.png`.
  - expand path (`wm size reset` -> physical `1216x2640`): `viewport 347x754`, stage `top=90 bottom=589 height=499`, stage child `height=499`, `.wterm` `height=499` / `clientHeight=499`, 35 DOM rows with `data-terminal-index` 14686..14715 (continuous), no `data-terminal-line-discontinuous`. Evidence: `android/evidence/2026-08-18/terminal-container-shrink-black/expand-after-1216x2640.png` (sha256 `13f540a9d0400abd02edc0f64e4af0194df3a14d66dc72de8cabb7a0235b123c`).
  - WebView CDP probe: `adb forward tcp:19222 localabstract:webview_devtools_remote_<pid>` then `ws://127.0.0.1:19222/devtools/page/<id>`. `pnpm run build:android` was intentionally skipped because it bumps version and publishes to public Relay; no OTA release was performed in this slice.
- Reusable rule added to `.agents/skills/terminal-buffer-truth/SKILL.md` (2026-08-18 terminal container shrink boundary): any shrink/expand regression must verify the real geometry chain `TerminalStageShell -> PaneStage -> .wterm -> ResizeObserver/clientHeight -> renderer viewport demand`; the shell stage wrapper must be a computable-height column flex container; `TerminalView` is the only owner of `viewportRows`; page/pane orchestration is forbidden from reading `buffer.rows` or `buffer.endIndex` to fabricate renderer demand; shrink verification must include `clientHeight > 0`, viewportRows following height, continuous absolute tail rows, and no `data-terminal-line-discontinuous`.
- DSH review completed on `2026-08-19`: task `terminal-container-shrink-black-20260818-dsh-r1`, provider `opencode-go`, model `deepseek-v4-flash`, final verdict `VERDICT: PASS`, no P0/P1, evidence `/Users/fanzhang/.dsh/reviews/terminal-container-shrink-black-20260818-dsh-r1/review.final.md`. Two non-blocking P2 findings remain: no machine-checkable anti-regression gate yet for page-level `buffer.rows/endIndex -> viewport demand` fabrication, and inactive-pane first-frame viewport is now renderer-measurement-dependent (recommend a real DOM measurement timing test). The fix was merged into main as local commit `1f2db762` (amendable before push) while unrelated dirty AppSDK/logo/AndroidConnectionService changes were left untouched.

## 2026-08-19 remote-window stream architecture remediation baseline

- `desktop.remote_window_stream` now uses daemon-published `RemoteWindowCanvasLayoutV1.layoutGeneration` as the only canvas/layout truth for both capture projection and input mapping. Client-side 1920x1080 layout construction and daemon SDP rewrite fallback were physically removed; quality uses a revisioned requested → applied/rejected transaction and one focus/overview budget with capture cadence updates.
- Daemon responsibilities are split into registered owners for catalog runtime, stream session cleanup, canvas layout, quality/cadence, input policy, capture, and gateway routing. Client responsibilities are split into registered owners for catalog cache/watchdog, receiver playback epoch/binding, ABR/ACK, viewport/IME geometry, focus switch transaction, composite canvas projection, screenshot thumbnails, picker/app-switch/More/video/toolbar/diagnostics views, plus the pure touch-action runtime. `RemoteWindowOverlay.tsx` is an 8-line facade; `RemoteWindowOverlayController.tsx` is bounded by a 3,200-line architecture gate and measured 3,078 lines at this baseline.
- Consolidated static/runtime evidence after the split: worktree and main both pass typecheck, architecture gates 100/100, direct Vite production build, `git diff --check`, and the complete `remote-window` path corpus in a single fork (23 files, 213/213). Default threaded Vitest may SIGSEGV during native `@roamhq/wrtc` teardown; the equivalent single-fork corpus is the verified invocation.
- Installed/live closure is not established in the managed sandbox: ADB smartsocket and Gradle lock-contention sockets are denied, daemon install cannot update `~/.local/bin`, direct daemon execution cannot access the tmux socket, and the installed launchd wrapper has an unmatched quote. Do not claim daemon/APK identity, real Android UI/input/weak-network/cleanup, DSH Review, commit, or push until those gates run in an authorized host context.
- AppSDK lock truth remains `appsdk 0.1.3` SHA `e3c36ae25c94d0c01c81cfe084fac7de8dc577f5ba3b8f91ae18b9d0587631a5`. On 2026-08-19 the mutable global `/Users/fanzhang/.local/bin/appsdk` changed to `appsdk 0.1.4` SHA `c26e500e34c129b331543925943a0a07bcf2c3320bb40eaf4ef184dc8df6d560`; canonical build correctly fails `APPSDK_BINARY_DIGEST_MISMATCH`. Remote-window work must not update the lock or bypass that gate.

## 2026-08-19 AppSDK local-install correction

- Jason clarified that AppSDK is intentionally consumed from the local installation; a GitHub release is not a prerequisite for a local version migration. Do not infer a missing remote release as a blocker when `/Users/fanzhang/.local/bin/appsdk` is the declared binary source. For a requested local upgrade, update the project bundle/lock and local governance contracts, run the local binary gates, and report any separate migration-record or CI artifact gap without blocking on remote publication.

## 2026-08-21 channel-replay stale-intent regression closeout

- Root cause of "kill tmux session → client cannot reconnect": daemon sends `mux-channel-closed` after session death, but `AndroidConnectionService.handleChannelClosed()` only dispatched to the state machine and left the dead `ChannelIntent` in `desiredChannels` with `opened=true`. On reconnect, `findOpenedChannelForIdentity()` matched this stale entry and returned its channelId, so the WebView reused a dead wire channel with no recovery path.
- Fix (commit `cc62f012`, branch merged into main as `0ec5b846`): `handleChannelClosed()` now calls `desiredChannels.remove(channelId)` before dispatching; `sendChannelOpen()` adds an idempotent same-channel reuse path plus a WebView-recreation path that finds an already-opened channel for the same backend-qualified identity (sessionName + options.backend) and republishes CHANNEL_OPENED without a redundant mux-channel-open frame on the wire.
- DSH review round 1 (task `2026-08-21-android-conn-svc-review`) verdict: PASS for documented bug, REWORK for threading. P1: plain LinkedHashMap accessed from OkHttp dispatcher thread (handleChannelClosed) and worker Handler thread (sendChannelOpen) can throw ConcurrentModificationException during iteration. P1 ordering caveat: closeStaleChannelsForSession ran before desiredChannels.put. P2: reuse path drained only reusable.channelId pendingFrames, not command.channelId.
- DSH review round 2 fixes (commits `3039c91c` and `f9fd29d3`): desiredChannels is now Collections.synchronizedMap(new LinkedHashMap<>()); all four iteration sites (findOpenedChannelForIdentity / closeStaleChannelsForSession / replayDesiredChannels / createNotification) wrapped in synchronized(desiredChannels) blocks; sendChannelOpen reordered so desiredChannels.put happens before closeStaleChannelsForSession; WebView-recreation drain extended to also flush command.channelId pendingFrames; webViewRecreationReusesExistingOpenedChannelWithoutWireOpen test extended to queue a frame under the requested id first and assert it drains to wire.
- Verification evidence: 10/10 transport unit tests pass (`./gradlew :app:testDebugUnitTest --tests com.zterm.android.AndroidConnectionServiceTransportTest`), full unit suite pass, type-check PASS, vite production build PASS, Gradle assembleDebug PASS. OTA bundle 0.1.3.2687 (versionCode 1100026870, sha256 9bf806abd3d19421519384d7ca0e401773cff56f082af1527466e3c9c1d9f38d) published via prepare-update-bundle.mjs and verify-update-bundle.mjs all 18 checks green. Device was offline during build — live smoke test (kill session → reconnect works) is still pending until device comes back online.

## 2026-08-24 daemon permission preflight fallback removed

- **Problem**: `prime_daemon_install_permissions()` in `zterm-daemon.sh` and `prepare-global-daemon-release.sh` called `ZTERM_DAEMON_NATIVE` (the `zterm-daemon` swift binary wrapping `/usr/sbin/screencapture`) to test Screen Recording permission. This was a fallback path that bypassed the installed `ZTerm Remote Capture.app` TCC identity.
- **Fix**: Both scripts now export `ZTERM_DAEMON_CAPTURE_NATIVE` pointing to the installed `ZTerm Remote Capture.app` binary and run `--permission-probe` only. The probe binary checks `CGPreflightScreenCaptureAccess()` (with `CGRequestScreenCaptureAccess()` + poll loop for the async TCC prompt) and exits 0 on grant, 5 on deny. Missing binary throws `installed ScreenCaptureKit capture binary is required` before the probe.
- **Swift source**: `remote-window-screen-capture-script.ts` now exports `hasScreenCapturePermission()` and uses `--permission-probe` argument; the normal capture path also calls `hasScreenCapturePermission()` at startup.
- **Tests**: 109/109 focused, type-check, feature-registry 102/102.
- **Live**: Installed binary `--permission-probe` exits 0; daemon restarted; emulator large-refresh burst shows no black screen (47 rows continuous, 0 gaps, terminal region non_black=1.0).
- **2026-08-24 release identity unification**: Release packaging previously staged a raw standalone `support/zterm-remote-window-capture` binary alongside the app bundle, splitting TCC identity between preflight (raw helper) and runtime (app bundle). Both scripts now physically remove the raw artifact and bind all 5 entry points (foreground/direct/launchd/preflight × source+release) to one `.app` bundle binary via `REMOTE_WINDOW_CAPTURE_APP_BIN`. Verified by static gate (`uses one app-bundle capture identity for every source and release daemon entry`) and release archive containing only the `.app` executable at mode 0755.
- **2026-08-24 explicit failure path verified**: Isolated daemon on port 3444 with `ZTERM_DAEMON_CAPTURE_NATIVE=/nonexistent/capture-binary` returns typed `remote_window_stream_start_failed / focus-capture-start / spawn ENOENT` with no screenshot or legacy capture fallback. Emulator burst of 50 000 lines into session `zterm-black-probe-2`: 71 CDP samples over 4 s, zero zero-row samples, zero discontinuous samples, final first row `49956` proving real render, screenshot non_black fraction 0.9993.

## 2026-08-24 daemon-only Screen Recording identity supersedes Remote Capture

- This entry supersedes the earlier same-day `ZTerm Remote Capture.app` identity notes. The installed `zterm-daemon` is the sole Screen Recording TCC subject and the sole remote-window capture executable: install/release preflight runs `ZTERM_DAEMON_NATIVE --permission-probe`, and runtime spawns that same binary with `remote-window-capture`.
- The Remote Capture bundle path, standalone raw capture artifact, `ZTERM_DAEMON_CAPTURE_NATIVE`, runtime Swift compilation, and screenshot fallback are physically absent from the product path. Missing capability returns typed `remote_window_capture_binary_missing` at `platform-capability` before the capture factory; failure is allowed and downgrade is forbidden.
- Verified installed truth: `~/.zterm/bin/zterm-daemon --permission-probe` exits 0; launchd runner exports only `ZTERM_DAEMON_NATIVE=/Users/fanzhang/.zterm/bin/zterm-daemon`; focused tests 110/110, type-check, feature registry 102/102, release archive inspection, and isolated typed-failure probe pass.
...
## 2026-08-25 one-shot screenshot uses the installed daemon TCC subject

- `capture-screen` previously spawned `/usr/sbin/screencapture`, creating a second Screen Recording TCC subject even though remote-window streaming used the installed daemon binary. The subprocess was physically removed. Window captures now use `SCContentFilter(desktopIndependentWindow:)`; rectangle captures select the containing `SCDisplay` and use its display-relative `sourceRect`.
- Permission and capability failures are explicit terminal failures from the same installed `zterm-daemon`: permission exit 5, target/display not found exit 4, invalid selector combinations exit 64. No request/poll/retry, helper executable, environment-variable alternate binary, runtime compilation, or screenshot/terminal fallback is permitted.
- Installed/global service truth was rebuilt and restarted: health PID `48966`, ready; installed daemon SHA-256 starts `7c099511`. The installed binary permission probe exited 0 in 140 ms; a real rect capture produced a 120x80 RGBA PNG with SHA-256 `9ed1d1430acb9c89c1c38b1a8e3ed5fe64e1fc3cdcea6504cef9182c8d89a976`; invalid-window, absent-selector, and both-selector probes returned explicit failures without output.
