...
## 2026-07-07 Daemon mirror tail monotonic truth

- Daemon mirror capture must not publish a single tmux frame as authoritative truth. The writer must publish only when the snapshot matches current mirror truth or when consecutive captures in the same sync agree; unstable capture is a failed sync, not a successful fallback.
- tmux `history_size + paneRows` can shrink during alternate-screen/TUI transitions even though the daemon mirror is the same logical stream. Mirror `availableEndIndex/latestEndIndex` must stay monotonic for the same live mirror; use current mirror end as the lower bound for the next absolute tail anchor, then publish the stable captured rows at that tail window.
- Client sparse buffer must reject a forward-revision `availableEndIndex` that regresses below current `bufferTailEndIndex` unless an explicit reset/restart path is already in effect. Do not let stale daemon payloads re-anchor local visible tail backward.
- Regression gates: `terminal-mirror-capture.test.ts` must cover stable consecutive publish and alternate-screen reanchor; `terminal-buffer.test.ts` must cover forward-revision tail regression rejection; `daemon:mirror:close-loop` must pass `top-live` and `vim-live` source/client replay comparisons.

## 2026-07-07 Session group active projection truth

- Session group center projection is not active runtime truth. Drawer selection, center-slot assignment, and peek activation must not move the visible center or grant `TerminalView` input/resize/width owner until `uiSessionId`/`interactiveSession` confirms the target runtime active session.
- `TerminalPage` may hold a pending session-group activation intent, but slot replacement/focus changes must apply only after the app-layer active session catches up. This prevents false-active UI where the screen shows the new session while input/transport/render freshness still belongs to the previous session.
...
- Short-window TUI tests are not enough for “刷着刷着死” or偶发 freeze. The standard daemon gate must include same-session soak (`tui-soak`): no session switch, continuous clear-screen redraw, sampled frame number must keep increasing, no long gap without `buffer-sync`, and final shell-return replay must exactly match tmux truth.
- Android renderer gate must separately cover a single mounted `TerminalView` consuming many render-store frames without remounting. Passing daemon soak only proves tmux -> daemon/client replay; it does not prove WebView paint unless `TerminalView.dynamic-refresh.test.tsx` or real-device runtime/DOM evidence also shows frame advancement.
