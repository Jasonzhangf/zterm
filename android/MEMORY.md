...
- [2026-06-30] UI 入口语义收口：Connections 主页面不再负责 `New Session` / `New Tab` 创建，只保留账号登录/服务器概览入口；session/daemon 操作 owner 是 Terminal drawer。drawer host rail 列账号目录 daemon，底部 `New Session` 必须按当前选中机器调用 daemon `tmux-create-session` 创建空白 session 后打开，禁止再跳回通用 new-connection picker。
