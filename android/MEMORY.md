...
- [2026-07-20] Supersedes the earlier same-day remote-window fullscreen bitrate cap rule: fullscreen entry/default must now be `fullscreen`/20Mbps when the user has not manually selected a lower preset in the current overlay; floating preview still starts and stays low at effective `2mbps`. Fullscreen IME projection must auto-pan upward by measured QuickBar chrome (`bottomChromeInsetPx`) when keyboard lift is active, then reduce fullscreen bottom padding by the same amount; manual fullscreen pan remains available and must not be overridden after the user moves it. This remains Android `resource.remote_window_overlay` projection truth plus `remote-window-stream-quality-request`; do not change daemon capture, Mac coordinate truth, terminal renderer, tmux geometry, or remote scroll to compensate. Required gates: `RemoteWindowOverlay.test.tsx`, `remote-window-video-quality.test.ts`, `TerminalPage.remote-window-overlay.test.tsx`, remote-window runtime/daemon focused tests, `test:feature-registry`, `tsc --noEmit`, `build:android`.
...
- [2026-07-20] Remote-window effective bitrate is projection-capped in Android overlay, while the selected preset remains remembered per desktop window. Floating preview always requests `2mbps`; fullscreen at scale `<= 1.01` caps any higher remembered preset to `5mbps`; only zoomed fullscreen may apply the remembered high preset such as `20mbps`. Effective quality changes must use `remote-window-stream-quality-request` on the active stream and must not restart ScreenCaptureKit, WebRTC receiver, session transport, or coordinate truth. Required gates: `remote-window-video-quality.test.ts`, `RemoteWindowOverlay.test.tsx`, `TerminalPage.remote-window-overlay.test.tsx`, `test:feature-registry`, `tsc --noEmit`.
