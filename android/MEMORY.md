...
- [2026-06-28] session group layout axis 的默认真源是 aspect ratio，不是设备名：`width / height <= 0.7` 的窄竖屏强制 vertical，上下滚；宽竖屏默认 horizontal，可在 Settings 切回 vertical；landscape 永远 horizontal。该设置只影响 layout projection，不改 drawer 固定槽位和 session/tab/pane 真相。
