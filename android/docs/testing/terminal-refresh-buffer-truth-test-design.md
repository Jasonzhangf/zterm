...
- `session-context-buffer-runtime.test.ts` and `SessionContext.ws-refresh.test.tsx` prove an explicit foreground resume requests one authoritative visible-window `buffer-sync` even when the returned head has the same revision and end index. The request must be driven by the pending resume demand, and the resulting same-revision body apply must repaint the DOM without a tab switch; an unchanged head without an explicit refresh demand remains request-free.
...
- The black-box cases must cover:
  ...
  - foreground resume returns the same revision/end head, then the authoritative visible-window body response changes rows at that same revision; DOM must update without switching sessions, and the resume demand is consumed once.
