...
- `terminal-mirror-capture.test.ts` proves a tmux `history_size` shrink produces a new authoritative absolute tail instead of retaining the previous cache-sized anchor.
- `terminal-mirror-capture.test.ts` proves a WezTerm external backend's available-line hint is its pane row count, never a mixed rows-plus-columns geometry value.
- `buffer-head-state.test.ts` proves a strictly newer local revision does not make a stale head's lower end index an impossible-window failure, while an equal revision still honors a daemon range that has shrunk past the local window.
...
- `TerminalView.dynamic-refresh.test.tsx` and shared renderer tests prove a visible cursor may participate in follow anchoring only when the local tail is a blank lower pane; the first frame must show the prompt window instead of a blank tail, and cursor metadata alone must not repaint body rows.
- `TerminalView.dynamic-refresh.test.tsx` and shared renderer tests prove a large buffer shift in reading mode realigns native scroll when the padding delta exceeds the viewport, so the old viewport cannot remain parked in blank padding until a manual scroll.
...
- The black-box cases must cover:
  ...
  - scrollback shrink from a near-cache-sized window to a short authoritative tail,
  - daemon head end/revision reset while the client still holds the old absolute window,
