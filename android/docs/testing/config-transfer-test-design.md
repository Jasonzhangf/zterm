...
- `src/components/settings/AppUpdateSection.test.tsx` proves Settings exposes the config transfer actions and marks the daemon shortcut as `server-connected` rather than an explicit user-saved URL.
