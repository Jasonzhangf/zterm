...
8. Input return policy
   ...
   - Unzoomed touch drag must not stream raw pointer moves; at release it must emit one Android app-window `kind=gesture`, `gesture=swipe`, `phase=end` action containing mapped start/end coordinates, tuning-scaled pixel deltas, duration, and velocity. This remains true when fullscreen has an IME bottom inset. Mouse/trackpad wheel remains `kind=scroll`. Zoomed fullscreen one-finger drag must pan locally and must not emit remote pointer drag. Zoomed fullscreen two-finger vertical movement must emit focus-first pixel `scroll` actions from the midpoint without moving the local projection; the tuning gate must prove amplitude presets change only delta magnitude and direction inversion flips only the delta sign while preserving mapped coordinates.
