...
6. Android overlay maps video-surface pointer/key/scroll events to `remote-window-input` intents. Unzoomed touch tap emits pointer down/up; unzoomed touch drag emits pointer down when the drag threshold is crossed, absolute pointer moves while dragging, and pointer up on release; mouse/trackpad wheel input remains pixel scroll, and zoomed fullscreen drag remains local pan. The daemon implements generic `bring-to-focus + os-event` pointer/scroll/key injection for active streams and keeps legacy gesture acceptance inside the daemon input owner only. Floating/fullscreen screenshot is a separate remote-screenshot/file-download request carrying the selected target manifest; it is not input and must not emit focus.
...
8. Input return policy
   ...
   - Page-level phone logic gate: render real `TerminalPage`, open the remote-window picker, select an interactive app-window, replay touch pointer tap and unzoomed touch drag on `remote-window-video-surface`, and assert the events leave the page through `onSendRemoteWindowInput` as focus-first pointer down/up for tap and focus-first pointer down/move/up for drag. The same gate must assert terminal input is not called, so QuickBar/terminal focus routing cannot hide a swallowed video-surface touch path.
   - Unzoomed touch drag must emit pointer down at the drag start point, absolute pointer moves at the mapped target coordinates, and pointer up at the release point; it must not emit Android app-window `kind=gesture` or delayed release-only commands. This remains true when fullscreen has an IME bottom inset. Mouse/trackpad wheel remains `kind=scroll`. Zoomed fullscreen drag must pan locally and must not emit remote pointer drag.
...
6. Generic app focus and scroll proof
   ...
   - Through the live daemon WebSocket protocol, start a real app-window stream against a marked AppKit probe window, send pointer down/move/up, pixel scroll, and key down/up over `remote-window-input`, and assert both protocol `accepted=true` and probe stdout events are observed. Use `pnpm --dir android exec tsx scripts/remote-window-live-input-probe.ts`; the probe must launch a temporary `.app` bundle so the daemon tests normal macOS app focus semantics rather than an unbundled Swift script. A separate daemon compatibility assertion may still cover the legacy `gesture/swipe` branch, but Android app-window touch gates must assert no gesture payload is emitted.
...
4. Input
   ...
   - A single-finger drag on an unzoomed floating/fullscreen video sends absolute pointer down/move/up to the selected target; it is not converted into a delayed gesture/swipe command.
