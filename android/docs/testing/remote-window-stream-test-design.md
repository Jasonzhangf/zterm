...
6. Android overlay now maps video-surface pointer/key/scroll/gesture events to `remote-window-input` intents. Unzoomed touch drag emits one recognized `gesture/swipe` command on release, mouse/trackpad wheel input remains pixel scroll, and zoomed fullscreen drag remains local pan. The daemon implements generic `bring-to-focus + os-event` pointer/scroll/gesture/key injection for active streams. Floating/fullscreen screenshot is a separate remote-screenshot/file-download request carrying the selected target manifest; it is not input and must not emit focus.
...
6. Overlay state machine
   ...
   - Current slice: unsupported iTerm2 pane `tmux-input` / `iterm2-api` targets are selectable for video but read-only for input; they do not publish `RemoteWindowInputContext`, do not send pointer/scroll/gesture/key payloads, and leave QuickBar routing on the terminal path.
...
8. Input return policy
   ...
   - Operation-scoped focus is mandatory: selecting a target, stream start, receiver attach, fullscreen entry, IME lift, pinch zoom, and local zoom-pan must not send `focus`; every actual remote pointer/gesture/wheel/key/QuickBar/IME event must be immediately preceded by same stream/target `focus`.
   - Screenshot is a non-input operation: floating/fullscreen screenshot button must send no `focus`, must call the existing remote screenshot request owner with `target.kind=remote-window`, must save the returned PNG on Android, and daemon must reject invalid target rect/window ids instead of falling back to full-screen capture.
   ...
   - Unzoomed touch drag must emit exactly one `kind=gesture`, `gesture=swipe`, `phase=end`, `unit=pixel` command on release with start/end target coordinates and aggregate deltas, without emitting pointer drag or incremental scroll during movement. This remains true when fullscreen has an IME bottom inset. Mouse/trackpad wheel remains `kind=scroll`. Zoomed fullscreen drag must pan locally and must not emit a remote gesture.
...
6. Generic app focus and scroll proof
   ...
   - Through the live daemon WebSocket protocol, start a real app-window stream against a marked AppKit probe window, send pointer down/up, pixel scroll, gesture/swipe, and key down/up over `remote-window-input`, and assert both protocol `accepted=true` and probe stdout events are observed. Use `pnpm --dir android exec tsx scripts/remote-window-live-input-probe.ts`; the probe must launch a temporary `.app` bundle so the daemon tests normal macOS app focus semantics rather than an unbundled Swift script.
...
4. Input
   ...
   - A single-finger vertical drag on an unzoomed floating/fullscreen video sends one recognized gesture/swipe command to the selected target; no pointer drag or incremental scroll is emitted for the same touch gesture.
