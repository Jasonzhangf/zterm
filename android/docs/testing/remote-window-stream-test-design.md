...
2. The picker requests the daemon `RemoteWindowStreamTargetManifest[]` catalog, groups same-app app-window rows through `WindowGroupLayout`, keeps every concrete window as its own child target container, keeps iTerm2 pane rows collapsed behind an explicit group until expansion, and surfaces explicit partial/top-level errors. In a same-app group, child click only promotes the large primary window; clicking the primary selects that concrete target.
