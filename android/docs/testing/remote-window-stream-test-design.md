...
8. Input return policy
   ...
   - Unzoomed touch drag must emit `kind=scroll`, `unit=pixel`, target coordinates, and incremental deltas during movement without also emitting pointer drag. This remains true when fullscreen has an IME bottom inset. Zoomed fullscreen drag must pan locally and must not emit remote scroll.
