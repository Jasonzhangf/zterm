...
3. Selecting one manifest locks a floating overlay shell to that target; the floating video surface sizes from the selected manifest crop aspect ratio before stream start and from daemon `started.capture.frameWidth/frameHeight` after stream start, toolbar drag repositions only the floating projection, resize keeps the toolbar reachable, the explicit fullscreen button or double tap/double click on the video surface enters fullscreen, Back/minimize returns to floating, and close invalidates the UI state. Fullscreen defaults to aspect-fit complete display. The explicit "fill" action sends a daemon `window-resize` request for the selected remote target and keeps local drawing/input projection aspect-fit.
...
6. Android overlay maps video-surface pointer/key/scroll events to `remote-window-input` intents. Unzoomed touch tap emits pointer down/up; unzoomed touch drag emits no raw move stream and emits one `gesture/swipe` action at release with start/end coordinates, delta, duration, and velocity; mouse/trackpad wheel input remains pixel scroll, and zoomed fullscreen drag remains local pan. The daemon implements generic `bring-to-focus + os-event` pointer/scroll/gesture/key injection for active streams and performs the current-frontmost/focused check inline before every real input action. Client-sent `focus` is a compatibility/explicit bring-front intent, not the only path that makes later input safe. Floating/fullscreen screenshot is a separate remote-screenshot/file-download request carrying the selected target manifest; it is not input and must not emit focus.
...
8. Input return policy
   ...
   - Page-level phone logic gate: render real `TerminalPage`, open the remote-window picker, select an interactive app-window, replay touch pointer tap and unzoomed touch drag on `remote-window-video-surface`, and assert the events leave the page through `onSendRemoteWindowInput` as pointer down/up for tap and one `gesture/swipe` action for drag, with no sends during pointer move and with any compatibility focus intents treated as non-authoritative. The same gate must assert terminal input is not called, so QuickBar/terminal focus routing cannot hide a swallowed video-surface touch path.
   ...
   - Unzoomed touch drag must not stream raw pointer moves; at release it must emit one Android app-window `kind=gesture`, `gesture=swipe`, `phase=end` action containing mapped start/end coordinates, absolute visible-source pixel deltas, duration, and velocity. This remains true when fullscreen has an IME bottom inset. Mouse/trackpad wheel remains `kind=scroll`. Zoomed fullscreen one-finger drag must pan locally and must not emit remote pointer drag. In every target-locked projection mode, two-finger vertical movement must emit pixel `scroll` actions from the midpoint without moving the local projection or entering pinch zoom. Two-finger scroll delta is proportional to local midpoint movement over the rendered surface and capped per event by the selected 1/8, 1/4, 1/2, or 1-screen preset; the preset is not a fixed jump size. Default direction is inverted; the direction toggle flips only delta sign while preserving mapped coordinates. Pinch zoom is fullscreen-only and requires stronger two-finger distance/scale expansion plus both fingers moving along the start-axis. The negative gate must prove near-parallel vertical movement and diagonal vertical movement with large distance change still remain scroll when the pinch axis intent is not clear, while a real half-started pinch does not emit a stray horizontal scroll before the second finger confirms pinch.
   ...
   - The daemon owns a persistent Swift input helper for a selected stream/runtime. Interactive app-window stream start must warm and readiness-check the helper without emitting focus/input, so helper cold start is not charged to the first realtime user operation. Pointer, scroll, gesture, and key sequences must not compile a fresh `swift -e` process per event. Each real input action must first check live frontmost PID plus matched AX focused window and only focus/raise when needed. Redundant same-target `focus` requests immediately followed by real input must be coalesced so phone-style compatibility focus bursts cannot block the helper queue until the following pointer/key operation becomes stale. Queued real input older than one second must still be dropped before helper injection.
   ...
   - Local action loopback is the required bridge before Android real-device proof: the live probe launches a temporary AppKit `.app`, starts a real app-window stream, maps local actions `tap`, `drag`, `scroll`, and `key` into remote-window input payloads, and asserts AppKit stdout sees mouse down/up, mouse dragged, scroll, and key markers. The same probe must run raw WebSocket and mux-channel paths with `ZTERM_REMOTE_WINDOW_PROBE_CLIENT_CLOCK_OFFSET_MS=-60000` so phone/Mac clock skew cannot be hidden by same-host tests, and with `ZTERM_REMOTE_WINDOW_PROBE_BURST=1` so phone/WebView behavior that sends compatibility focus + pointer/key/text back-to-back cannot be hidden by a serial ACK-after-each-event probe. A serial live probe proves only protocol reachability; it is not a phone-parity gate for remote-window input stale bugs.
