...
3. Selecting one manifest locks a floating overlay shell to that target; the floating video surface sizes from the selected manifest crop aspect ratio before stream start and from daemon `started.capture.frameWidth/frameHeight` after stream start, same-app sibling windows render as child tiles inside the same locked video container with a top rail in portrait and side rail in landscape, each non-active child shows a screenshot thumbnail so the user can identify the window before switching, toolbar drag repositions only the floating projection, resize keeps the toolbar reachable, the explicit fullscreen button or double tap/double click on the video surface enters fullscreen, Back/minimize returns to floating, and close invalidates the UI state. Fullscreen defaults to aspect-fit complete display and may zoom in, but it must never shrink below fit; there is no minimap/viewport overlay. The explicit "fill" action sends a daemon `window-resize` request for the selected remote target and keeps local drawing/input projection aspect-fit.
...
6. Android overlay maps video-surface input/key/scroll events to `remote-window-input` intents. Touch tap and mouse click emit one `click` action; unzoomed touch/mouse drag emits no raw move stream and emits one `gesture/swipe` action at release with start/end coordinates, delta, duration, and velocity; two-finger vertical movement emits one release-time `gesture/swipe` action from the midpoint path instead of streaming move-time scroll frames; mouse/trackpad wheel input remains one pixel `scroll` action, and zoomed fullscreen drag remains local pan. The daemon implements generic `bring-to-focus + os-event` click/legacy-pointer/scroll/gesture/key injection for active streams and performs the current-frontmost/focused check inline before every real input action. Client-sent `focus` is only an explicit/compatibility bring-front intent; ordinary click/gesture/wheel/key/QuickBar/IME paths must send just the user action record, with no client focus prefix. Floating/fullscreen screenshot is a separate remote-screenshot/file-download request carrying the selected target manifest; it is not input and must not emit focus.
...
8. Video bitrate is stream-local quality control: stream start carries the effective selected bitrate config, untouched defaults are conservative `2mbps`, floating preview starts low at `2mbps`, Android phone fullscreen does not upgrade bitrate by itself, manual selector choices are remembered per target/resolution and may raise fullscreen baseline, and the overlay selector sends `remote-window-stream-quality-request` for an active stream. The daemon may update only the existing WebRTC sender encoding entries without changing the `encodings` array structure. Effective quality may be capped by measured browser network hints and WebRTC `getStats()` samples; regular presets default to 30 FPS, desktop-fullscreen preset caps at 60 FPS, stream start passes that requested max FPS into ScreenCaptureKit, weak stats require consecutive weak samples before downgrade to one-quarter bitrate and half FPS, and restore happens only after a stable window using per-sample deltas for cumulative dropped/freeze/jitter counters. An empty sender `encodings` array must not fail stream startup or produce a false `capture.maxBitrateBps`; a later live quality update against the same unsupported sender must fail explicitly. The update must not restart capture or rebuild transport.
...
6. Overlay state machine
   ...
   - Current slice: fullscreen button defaults to aspect-fit letterbox mode; the explicit fill control sends one remote target `window-resize` request that keeps the current desktop target width and computes target height from the phone surface aspect ratio. Local drawing and input mapping remain aspect-fit; the control must not restart capture, rebuild transport, or fake a local cover/crop as "fill".
...
8. Input return policy
   ...
   - Touch tap must not be represented as pointer down/up; at release it must emit one Android app-window `kind=click` action with mapped coordinates. Unzoomed touch drag must not stream raw pointer moves; at release it must emit one Android app-window `kind=gesture`, `gesture=swipe`, `phase=end` action containing mapped start/end coordinates, absolute visible-source pixel deltas, duration, and velocity. This remains true when fullscreen has an IME bottom inset. Mouse/trackpad wheel remains `kind=scroll`. Zoomed fullscreen one-finger drag must pan locally and must not emit remote pointer drag. In every target-locked projection mode, two-finger vertical movement must emit one release-time `gesture/swipe` action from the midpoint without moving the local projection or entering pinch zoom. Two-finger gesture delta is proportional to local midpoint movement over the rendered surface and capped per action by the selected 1/8, 1/4, 1/2, or 1-screen preset. Default direction is inverted; the direction toggle flips only delta sign while preserving mapped coordinates. Pinch zoom is fullscreen-only and requires stronger two-finger distance/scale expansion plus both fingers moving along the start-axis. The negative gate must prove near-parallel vertical movement and diagonal vertical movement with large distance change still remain gesture-scroll when the pinch axis intent is not clear, while a real half-started pinch does not emit a stray horizontal scroll before the second finger confirms pinch.
