...
12. Daemon stream owner
   ...
   - A capture frame that arrives before `setLocalDescription` and WebRTC media connection readiness is not counted as sent and must not emit `streaming`; the daemon stores only the latest pending frame, flushes it once after the peer becomes connected, and drops it on stop/cleanup.
