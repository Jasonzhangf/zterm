...
7. Catalog transport readiness
   ...
   - Daemon runtime startup warms one canonical full app-window+iTerm2 snapshot and refreshes it on its own cadence. A client read must not rerun Swift/iTerm2 enumeration; it projects the current snapshot by requested source set and surfaces real enumeration/tmux errors instead of substituting an empty catalog. A read before the first snapshot completes must wait for that daemon-owned refresh or fail with explicit `remote_window_catalog_not_ready`; `forceRefresh` is a compatibility no-op that must not start a second enumeration.
   ...
   - Overlay projection keeps the last successful catalog for the current active session and renders those rows immediately on picker reopen or manual refresh. The first open before the daemon snapshot exists may show a compact loading row; later opens must not blank the target list. The Refresh button re-reads the daemon snapshot without creating a second catalog truth or triggering live enumeration.
...
8. Input return policy
   ...
   - Touch tap must not be represented as pointer down/up; at release it emits one Android app-window `kind=click` action with mapped coordinates when unzoomed. At zoomed scale, one-finger movement and tap are local-only container operations and emit no remote action. A stationary zoomed single-finger 500 ms hold promotes to reliable left-button `down -> move* -> up`; movement before the hold keeps local pan and cancels promotion. Two-finger coherent vertical motion emits remote scroll at every zoom level; a committed pinch remains local and cannot switch to scroll, and a committed scroll cannot switch to pinch. Pointer cancel after a remote down emits a reliable release. Mouse Emulation drag emits pointer `down -> move* -> up`. Negative gates prove no zoomed single-finger scroll/click, no scroll cursor movement, no gesture-mode switch after commit, no half-pinch stray scroll/click, and no duplicated long-press recognition.
