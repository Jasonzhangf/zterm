...
1. Export starts from all local `EditableHost` / `Host` connection truths by default.
2. Shared owner normalizes bridge endpoint fields and builds one canonical multi-host payload.
...
5. Import parses the link or raw encoded payload and returns explicit success hosts or explicit error.
6. UI import must call host storage `upsertHost` for every parsed host; pages must not write `localStorage` directly.
...
8. In the real add sheet, default sharing scope is all saved connections; single-connection sharing is only an optional narrowed scope.

## White-Box Plan

- `packages/shared/src/connection/connection-config-share.test.ts` covers canonical multi-host payload build, app link, web link, raw encoded payload import, endpoint normalization, secret stripping, malformed input, unsupported URL, and invalid host identity.
- `src/components/tmux/TmuxSessionPickerSheet.test.tsx` proves the real FAB/add-flow sheet exposes paste import, QR image scan entry, all-connections share by default, optional single-connection narrowed selection, canonical share link, and QR rendering.
...
- Add sheet export tests prove multiple saved connections are encoded into one default QR/link.
