...
8. The no-argument canonical entry must invoke build allocation without expanding an empty shell array; explicit resume passes exactly `--resume N`. Both paths must remain valid under the shipped macOS Bash 3.2 with `set -u`.
...
- Positive: the ordinary no-argument build uses a scalar-safe allocation branch and reaches `bump-build-version.mjs` under Bash 3.2 nounset mode.
- Negative: the canonical script contains no empty `${array[@]}` expansion in its no-argument path; resume remains explicit and cannot silently allocate a new slot.
