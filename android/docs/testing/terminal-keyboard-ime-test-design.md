...
- Android WebView focus handoff positive path: before `ImeAnchor.show()` moves native focus to the hidden editor, `TerminalPage` clears any stale WebView `document.activeElement`; when native focus later returns to WebView, QuickBar press-start clears the restored non-editor shell focus before the browser can paint it as `:focus-visible`.
- Android WebView focus handoff negative path: when the QuickBar editor owns DOM focus, neither the native handoff guard nor QuickBar press-start may blur that input, textarea, or contenteditable editor.
...
- QuickBar regression path: ↑ and ↓ remain non-tab-stop pointer actions and continue emitting `\x1b[A` and `\x1b[B`; focus cleanup belongs to the shared `client.input_runtime` QuickBar press lifecycle, not status CSS, renderer, or daemon.
