...
10. Android Activity backgrounding must leave WebView timers running. While at least one retained Session exists, a native foreground service keeps the process schedulable with one partial WakeLock; it cannot create, close, replace, or evaluate control/data transports.
