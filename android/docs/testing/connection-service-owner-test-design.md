...
Android service projection is also part of this boundary: a WebView listener may attach after the native target is already `healthy`; the latest snapshot must replay one `mux-ready` projection so channel opening can continue without a second physical transport or JS handshake/heartbeat.
...
- Target-scoped control frames preserve `requestId` and typed `message` through `mux-target-message` -> service IPC -> native mux wire; malformed target messages are rejected without closing a healthy physical projection.
...
- `src/lib/android-connection-service-socket.test.ts` must prove a late JS listener receives one open event and one typed `mux-ready` replay from a healthy native snapshot.
- The same socket test must prove a service-owned socket never emits JS `mux-hello` or owns JS heartbeat/probe timers.
- UI socket close remains projection-only; target release stays an explicit service command owned by target/session lifecycle.
...
| Late listener receives current `mux-ready` projection | JS does not send a second `mux-hello` or heartbeat |
