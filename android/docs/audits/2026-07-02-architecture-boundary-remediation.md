...
- Allowed side effect: active apply may only run `resize-window -x <narrowest cols>`; final release must restore the captured baseline `cols + rows`, then run `set-window-option -u window-size` last. All of these remain inside `applyAdaptiveTmuxWidth()` / `releaseAdaptiveTmuxWidth()`.
