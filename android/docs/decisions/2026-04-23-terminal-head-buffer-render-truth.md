...
- frame assembly 的资源边界由 shared protocol 唯一声明：最多 `4096` 行 span、`512` chunks、`64 MiB` retained serialized bytes、`15s` incomplete lifetime。client ingress 在写入 retained map 前拒绝越界；buffer-head cadence 或下一次 chunk 到达时发现 incomplete frame 到期，都必须释放 incomplete chunks、写入 `frame-assembly-expired` error truth，并只通过既有 exact-range repair owner 请求一次。禁止依赖 session close、后续 buffer-head 或下一次 chunk 才释放过期 frame。
...
- explicit resume 期间，若 lower-revision fresh body 早于 head 到达，只有完整 authoritative tail 可以建立新 revision epoch。unchunked payload 必须显式通过 `availableStartIndex/availableEndIndex` 与 body 的连续行覆盖共同证明它覆盖当前 tail；chunked payload 必须先按 frame identity 完成 assembly，再以完整 frame window 证明覆盖。dense 但只覆盖局部 available window 的 payload、缺字段或 malformed 的 payload 都只能按 stale drop 处理，不得清空 pending frame、repair ledger 或 local buffer truth；已在 staging 的 body-first frame 后续 malformed/interleave/resource-limit输入也只拒绝该输入并保留已收 chunks，唯有过期候选走 exact-range repair。
