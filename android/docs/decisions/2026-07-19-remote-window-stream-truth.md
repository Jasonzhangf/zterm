...
6. Use the explicit fullscreen button next to close, or double tap the video surface, to enter fullscreen. Fullscreen keeps local drawing/input aspect-fit and defaults to sending the remote target resize fill request once a stream surface is available.
...
9. Fullscreen preserves source aspect ratio locally. The default fill strategy sends a remote target resize request for the current phone surface instead of doing a local cover/crop; the manual fill control reissues the same owner request if needed. Pinch zoom can only enlarge above fit, zoomed fullscreen allows one-finger pan, and there is no minimap/viewport overlay.
...
12. Target-locked two-finger vertical movement is realtime touch-mode scroll, not release-time gesture: once both fingers show coherent same-direction vertical motion, Android emits bounded pixel `scroll` actions on pointer move at the touch midpoint and emits no final remote action on pointer up. Two-finger scroll is vertical-only; horizontal midpoint drift during possible pinch expansion must not inject horizontal scroll. Pinch zoom is local fullscreen projection only and requires clear relative distance / opposite-axis intent.
...
Live quality control is an acknowledged stream-group transaction. A request carries `streamGroupId` and monotonic `revision`; success returns the requested and actually applied focus/overview configuration plus the one total group budget. The overview lane is capped at 20% and 1.5 Mbps, with the remaining budget assigned to focus. Sender encoding parameters and capture cadence are changed together; any lane failure rolls every touched lane back, does not advance the applied revision, and returns an explicit rejection. The client displays requested/applied/rejected separately and never projects requested quality as applied before the matching ACK. ABR uses a bounded degradation ladder and restores only one level after a stable window.

Gesture recognition has one client owner. Direct Touch locks a two-finger interaction to scroll or pinch once committed; lifting one finger transitions to local pan when zoomed, or a tap-suppressed continuation when unzoomed. Mouse Emulation drag emits pointer down, realtime move, and pointer up. The first direct-touch scroll event sets `moveCursor=false`. Runtime dispatch consumes the recognized action record and must not implement a second long-press recognizer. Primary controls expose mode, screenshot, keyboard, and More with 48-by-48 minimum hit targets; low-frequency display and quality controls live under More, and developer diagnostics remain collapsed separately from user status.
