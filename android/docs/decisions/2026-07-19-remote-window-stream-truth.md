...
Current status is anchored for app-window catalog, collapsed same-app picker rows plus active video-layer sibling window switching, default-collapsed iTerm2 picker grouping, real ScreenCaptureKit/WebRTC video, Android floating/fullscreen projection with safe-area top chrome and IME bottom-inset lift, toolbar-reachable floating resize, fullscreen aspect-fit default plus explicit remote target resize fill request, fullscreen zoom/pan/minimap, route-derived ICE for remote-window video, per-window bitrate start/update control, focus-aware image paste routing, raw Android IME text routing, touch tap to absolute pointer down/up, drag to one release-time gesture/swipe action, target-locked two-finger scroll, plus wheel pixel-scroll intent for supported app-window targets, read-only projection for unsupported iTerm pane input routes, generic `bring-to-focus + AXRaise + os-event` pointer/scroll/key injection with daemon-only legacy gesture compatibility, and remote-window screenshot requests that reuse the existing remote screenshot/file-download path without focus. Remaining live completion gaps are Android real-device input replay after this interaction slice, live remote-window screenshot proof on installed daemon, and iTerm2-pane stream/input proof.
