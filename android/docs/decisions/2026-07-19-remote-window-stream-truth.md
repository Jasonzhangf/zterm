...
9. Fullscreen preserves source aspect ratio locally. The default fill strategy sends a remote target resize request for the current phone surface instead of doing a local cover/crop; the manual fill control reissues the same owner request if needed. Pinch zoom can only enlarge above fit, zoomed fullscreen local pan requires coherent two-finger movement, and there is no minimap/viewport overlay.
10. Opening the Android IME lifts the target-locked floating preview by the same bottom inset as the QuickBar so the preview remains visible above the keyboard. In fullscreen, the same bottom inset is consumed as overlay padding and `bottomChromeInsetPx` may auto-lift the projection, but one-finger touch remains remote input at both 1x and zoomed scale: tap sends one `click`, movement crossing 8 px emits realtime bounded pixel scroll, movement after a 250 ms hold emits reliable remote drag, and a stationary 500 ms hold sends one right click.
11. Zoomed pointer-down does not pre-commit local pan. A committed one-finger scroll or hold-drag stays latched until release; a five-second drag and pointer-cancel both deliver a reliable remote release. Touch outside the rendered content rect returns no source point rather than clamping to a remote edge.
12. Target-locked two-finger coherent same-direction movement emits realtime remote scroll at 1x and local canvas pan while zoomed. Anti-parallel relative-distance change emits local pinch zoom. Once committed, scroll, pan, and pinch cannot switch or dual-dispatch during the same pointer sequence.
