...
Video quality is stream-local control, not transport lifecycle. Android may remember a bitrate preset per selected desktop window identity and seed defaults from source resolution, but the effective bitrate request is capped by the projection size: floating preview always starts/updates at `2mbps`, unzoomed fullscreen requests no more than `5mbps`, and zoomed fullscreen may use the remembered high preset. Changing bitrate or projection mode must not restart capture, recreate the receiver, rebuild the session transport, or change daemon coordinate truth. The daemon stream owner applies the requested sender bitrate only by preserving the WebRTC sender's existing `RTCRtpSendParameters.encodings` count/order and changing `maxBitrate`; it must not fabricate an encoding entry when `encodings` is empty. Empty sender encodings during stream start mean bitrate is explicitly not applied and `capture.maxBitrateBps` is omitted while video startup continues; the same condition during a live quality update is an explicit unsupported quality error. Unsupported or inconsistent bitrate payloads are rejected explicitly.
