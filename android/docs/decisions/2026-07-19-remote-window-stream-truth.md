...
The remote-window app catalog is daemon-wide projection data, not tmux-session data. The daemon keeps one canonical full app-window+iTerm2 snapshot and refreshes it on its own cadence; a client read is a read-time projection of that snapshot, so opening the picker or switching sessions never triggers live Swift/iTerm2 enumeration. The Android client keeps a 60-second projection cache keyed by `{ daemonHostId, bridgeHost, bridgePort, authToken }`, so switching between sessions on the same daemon reuses the last catalog rather than sending another full enumeration. A closed physical session transport is still an explicit error and cannot be hidden by stale catalog data. The cache does not create a second WebSocket and does not replace the existing `SessionContext` transport owner.
