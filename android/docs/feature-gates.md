...
- `relay.directory_ui`: prove Settings accepts only account/password for fixed `relay.codewhisper.cc`, persists no plaintext password, Home projects daemon devices/server rows without Session-group controls, and normal Home server-row entry goes through the session-open owner directly instead of stopping at the Session Picker; Session Picker/drawer remain the advanced Session projection owners.
