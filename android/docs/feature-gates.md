...
- `relay.directory_ui`: prove Settings accepts only account/password for fixed `relay.codewhisper.cc`, persists no plaintext password, Home projects daemon devices/server rows without Session-group controls, same-daemon saved direct rows merge Relay route candidates without replacing saved identity, rows with `relay-rtc` expose explicit Auto and Relay open intents, normal Home server-row entry goes through the session-open owner directly instead of stopping at the Session Picker, and Home server rows reuse a current-process open Session before creating any generated tmux session; Session Picker/drawer remain the advanced Session projection owners.
