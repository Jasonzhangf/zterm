...
- `connections.config_share`: prove connection share has one canonical shared payload/link owner, QR/copy/deep-link import consume the same payload, secrets are stripped, malformed input returns explicit errors, and UI import goes through host storage upsert instead of direct storage writes.
