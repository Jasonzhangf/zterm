...
flowchart TD
  ...
  App --> AndroidConnectionServiceIn01SetManualRoutePolicy["AndroidConnectionServiceIn01SetManualRoutePolicy (user command only)"]
  App --> AndroidConnectionServiceIn02BindTarget["AndroidConnectionServiceIn02BindTarget (typed command)"]
  App --> AndroidConnectionServiceIn03ReleaseTarget["AndroidConnectionServiceIn03ReleaseTarget (typed command)"]
  AndroidConnectionServiceIn01SetManualRoutePolicy --> AndroidConnectionServicePlugin["native AndroidConnectionServicePlugin"]
  ...
  AndroidConnectionServiceOut01Snapshot --> AndroidConnectionServiceSnapshot["src/lib/android-connection-service-snapshot.ts"]
  AndroidConnectionServiceSnapshot --> AndroidConnectionServiceRuntime["src/contexts/android-connection-service-runtime.ts (projection)"]
  ...
  ClientFileTransferDownloadIn02SessionProjection --> ClientFileTransferDownloadIn03SheetPersistence["src/lib/file-transfer-session-runtime.ts#createFileTransferSessionRuntime"]
  ClientFileTransferDownloadIn03SheetPersistence --> ClientFileTransferDownloadIn04NativeWriteBatch["src/lib/file-transfer-native-store-port.ts#createFileTransferDownloadStore"]
  ClientFileTransferDownloadIn04NativeWriteBatch --> ClientFileTransferDownloadIn05NativeWriteDispatch["src/lib/file-transfer-native-store-port.ts#persist"]
  ...
  ClientFileTransferDownloadPersistOut01BytesWritten --> ClientFileTransferDownloadPersistOut02VerifiedStat["src/lib/file-transfer-native-store-port.ts#complete"]
  ...
  ClientFileTransferDownloadIn06NativeStore --> ClientFileTransferDownloadErrorIn02NativeWriteFailure["src/lib/file-transfer-native-store-port.ts#persist"]
  ClientFileTransferDownloadPersistOut02VerifiedStat --> ClientFileTransferDownloadErrorIn03SizeMismatch["src/lib/file-transfer-native-store-port.ts#complete"]
  ...
  RemoteWindowOverlay -. "binding pending: smooth/quality desired profile" .-> RemoteWindowQualityControl["src/lib/remote-window-quality-controller.ts + src/components/terminal/useRemoteWindowQuality.ts"]
  RemoteWindowQualityControl -. "binding pending: one in-flight/latest desired" .-> RemoteWindowStreamQualityRuntime
  ...
  RemoteWindowTouchAction -. "binding pending: reliable or continuous" .-> RemoteWindowInputDeliveryClient["src/lib/remote-window-message-runtime.ts + src/contexts/session-context-remote-window-runtime.ts"]
  RemoteWindowInputDeliveryClient -. "binding pending: typed delivery control" .-> RemoteWindowInputRuntime
  RemoteWindowOverlay -. "binding pending: decoded-frame draw clock" .-> RemoteWindowFrameProjection["src/components/terminal/useRemoteWindowCompositeCanvas.ts"]
...
The dashed remote-window quality, input-delivery, and frame-projection edges are
Phase 0 target bindings from the 2026-08-30 amendment. They remain
`binding pending` until runtime symbols and paired gates pass; existing anchored
edges continue to describe current code and must be physically removed or
rebound when each target edge becomes active.
...
flowchart TD
  ...
  RemoteWindowStream -. "binding pending: one latest frame per lane" .-> RemoteWindowCaptureBackpressure["src/server/remote-window-stream-daemon.ts + src/server/remote-window-capture.ts"]
  ...
  RemoteWindowInputDelivery["src/server/remote-window-input-helper.ts + src/server/remote-window-stream-daemon.ts"] -. "binding pending: dedupe/age/barrier" .-> RemoteWindowInput
...
flowchart TD
  ...
  Npm --> WinShell
  GitProtectionSetup["scripts/setup/enable-local-protection.sh"] --> GitProtectionVerify["appsdk verify-git-main-protection ."]
```

Git main protection is a repository control path. Each clean owner worktree
installs the tracked hooks and runs `appsdk verify-git-main-protection .`.
It has no daemon, terminal, Active, or Protected business-resource edge.
...
| Remote window stream projection | `packages/shared/src/connection/protocol.ts`, `src/server/remote-window-stream-daemon.ts`, `src/server/terminal-message-runtime.ts`, `src/server/server.ts`, `src/server/terminal-file-transfer-binary-runtime.ts`, `src/components/terminal/RemoteWindowOverlay.tsx`, `src/components/terminal/RemoteWindowOverlayController.tsx`, `src/components/terminal/useRemoteWindowCompositeCanvas.ts`, `src/components/terminal/useRemoteWindowQuality.ts`, `src/lib/remote-window-quality-controller.ts`, `src/lib/remote-window-touch-action-runtime.ts`, `src/lib/remote-window-message-runtime.ts`, `src/lib/remote-window-receiver-runtime.ts`, `src/lib/remote-window-video-quality.ts`, `src/contexts/session-context-remote-window-runtime.ts`, `src/server/remote-window-quality.ts`, `src/server/remote-window-capture.ts`, `src/server/remote-window-screen-capture-script.ts`, `src/server/remote-window-input-helper.ts`, `docs/decisions/2026-08-30-remote-window-quality-gesture-control-amendment.md`; ScreenCaptureKit/WebRTC bindings remain anchored under `RemoteWindowMessageRuntime`, `RemoteWindowReceiver`, `RemoteWindowStreamQualityRuntime`, `RemoteWindowTouchAction`, `RemoteWindowCapture`, `RemoteWindowMedia`, `RemoteWindowInput`, and `RemoteWindowCleanup`. Target state adds single-flight/latest-wins smooth/quality control, exact no-op/in-place sender and `SCStream` diffs, cause-separated ABR, realtime 1x single-finger remote scroll, zoomed fullscreen single-finger no-op, zoomed floating local pan, zoomed two-finger remote scroll, held reliable drag/right-click/cancel release, Mouse Emulation pointer/wheel, reliable/continuous delivery separation, decoded-frame draw-once projection, and one-pending latest capture/conversion per lane. Control facts remain typed side-channel state and never enter video or input business metadata. |
...
| Release/update | `contracts/app-version.json`, `native/android/build.gradle`, `scripts/app-version.mjs`, `scripts/bump-build-version.mjs`, `native/android/app/build.gradle`, `scripts/build-android-debug.sh`, `scripts/verify-update-manifest-version.mjs`, `scripts/prepare-update-bundle.mjs`, `scripts/verify-update-bundle.mjs`, `scripts/verify-release-assets.mjs`; Android dependency resolution uses official Google Maven and Maven Central sources only; normal `N`, rollback `N.1`, and next normal `N+1` occupy strictly increasing Android version-code slots; an interrupted build resumes only with an explicit expected already-allocated `N`; Relay publication validates numeric manifest truth rather than JSON whitespace; Relay public update route and future `RelayPeerLease` idle-resume resource are authored in `src/traversal-relay/server.ts` and packaged by `scripts/prepare-relay-server-npm-package.mjs` with `ZTERM_TRAVERSAL_UPDATES_DIR` |
