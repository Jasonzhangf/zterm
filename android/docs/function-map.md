...
| `terminal.keyboard_ime` | `src/pages/terminal-keyboard-lift.ts`; `src/pages/TerminalPage.tsx`; `src/pages/TerminalPageStageShell.tsx` | IME lift, keyboard visibility, viewport safe-area, quickbar focus handoff, stage shell visual lift even when quickbar editor owns DOM focus, listener cleanup | `terminal-keyboard-lift.test.ts`; `TerminalPage.android-ime.test.tsx`; `TerminalPage.lifecycle-cleanup.test.tsx`; `TerminalPageStageShell.pane-stage.test.tsx` |
