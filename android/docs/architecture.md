...
- File Transfer UI：`client.file_browser` 同时拥有 `resource.file_browser_ui_contract`，`terminal.file-browser` UI slot 由 `FileBrowserUiPlugin` 经 plugin host 提供。App 注入现有 send/subscription 能力，由 file-browser session-port factory 固定 session 和 file scope；TerminalPage 只按 UI session intent 读取 typed port 并投影 open/mode，不持有 raw sender、active-session fallback 或 file scope 构造。FileTransferSheet 的 native I/O 仍是后续拆分项；本切片不宣称整体业务解耦完成。
...
- Session list truth：session 列表由 daemon 在内存中维护为 session catalog 真源，触发刷新只发生在 daemon create/close/rename session，或 daemon 主动检测到 backend-qualified session membership 变化时；目录发布只读取当前 catalog，不得触发刷新。显式 catalog refresh 失败必须使缓存快照失效并显式失败，读取方不得因失效而重新枚举，直到 daemon 侧显式 refresh 成功；周期检测枚举失败只记录失败并保留最后一次完整快照，不得污染或清空当前 catalog。不得由客户端实时 `list-sessions` 拉取作为刷新语义，也不得读取或写入持久化 session 列表。不得扫描任意 `/tmp`/`/private/tmp` socket，不得把 tmux attach client 参数、cwd、测试 session 或缓存内容当作 session。测试必须使用隔离的 tmux server，且不得污染默认 server。
...
- 输入 action 与 delivery control 物理隔离：Direct Touch 在 1x 的单指移动是 realtime remote pixel scroll；zoomed fullscreen 单指完全 no-op（不发 tap/hold/drag/scroll，也不平移本地 canvas），只有双指生效。1x 与 zoomed 的双指同向移动都是 realtime remote scroll，anti-parallel 距离变化才是本地 pinch。hold-drag/cancel release 属可靠 lane；连续 move/scroll 可合并且有 depth/age/rate 上限，可靠 click/down/up/key/barrier 不可丢。契约真源为 `docs/decisions/2026-08-30-remote-window-quality-gesture-control-amendment.md`。
