...
Status: static architecture slice first, then implemented as the daemon
mirror/control decision core in `android/native/dagpipe`. The graph JSON and
operator contracts below are the authoritative static DAG; the Rust crate
registers these operators and compiles both daemon graphs with the SDK.
...
- `pnpm --dir android test:dagpipe-phase1` runs cargo fmt/clippy/test,
  the N-API bridge parity tests, and the static Phase 0 gate.
- The daemon is restarted only after black-box parity tests pass.

## Runtime Claim Boundary

The daemon mirror publish and control dispatch graphs execute through the Rust
operators after Phase 1 wiring. Static graph validity alone is not runtime
closure; the separate cargo/parity tests and daemon restart evidence prove the
executable path.
