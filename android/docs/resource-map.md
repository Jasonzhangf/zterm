...
| Relay runtime | `resource.relay_peer_lease` | Relay may preserve an idle-timeout bounded peer/signaling lease for a daemon target route. It must not preserve tmux/session/UI truth. |
...
- `resource.relay_peer_lease` owns only an idle-timeout bounded Relay peer/signaling lease and resume token for one daemon target route. It may rebind a phone signaling socket before timeout, but it must not store terminal channel, subscriber, tmux, mirror, active-tab, foreground, viewport, or UI truth.
