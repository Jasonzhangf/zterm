...
- `resource.remote_window_overlay` owns only the Android picker/floating/fullscreen projection, fullscreen zoom/pan/minimap state, Back shrink, minimize, close intent, and explicit input intent emission. It does not compute macOS coordinates, read iTerm2 split trees, perform input injection, or own capture lifecycle.
