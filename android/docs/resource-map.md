...
- `resource.remote_window_overlay` owns only the Android picker/floating/fullscreen projection, source-aspect floating resize, fullscreen aspect-fit/aspect-fill display mode, fullscreen zoom/pan/minimap state, keyboard-open fullscreen letterbox pan, projection-sized effective bitrate selection, Back shrink, minimize, close intent, and explicit input intent emission. Aspect-fill cover is Android projection only: drawing and pointer normalization use the same actual content rect, and the daemon capture/coordinate truth remains unchanged. It does not compute macOS coordinates, read iTerm2 split trees, perform input injection, or own capture lifecycle.
