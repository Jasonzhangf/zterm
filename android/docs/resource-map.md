...
- `resource.daemon_session_catalog` owns backend session catalog construction, backend-qualified `sessionCatalog` rows, `list-sessions` dispatch, list-time idle facts, and the explicit daemon session-agent registration probe through `src/server/daemon-session-catalog-runtime.ts`; it consumes `resource.backend_session` and `resource.session_idle_facts` only, and must not own client active/session, mirror store, transport subscriber, renderer, or UI truth. Agent registration is daemon-local in this slice; no shared payload field is added until a separate control consumer contract exists.
