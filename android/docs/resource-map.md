...
- `resource.client_reliable_input_queue` is the only client owner for reliable terminal input seq, one in-flight frame, ACK application, bounded ACK-timeout retry, and physical-transport replacement retry. It compares the typed `resource.session_transport` socket identity locally and never writes retry control state into terminal input payloads.
