...
| Governance artifact | `resource.appsdk_compiled_artifact` | The pinned AppSDK compiler owns the compiled manifest and immutable web bundle. `module.compiled.json.artifact_hash` is the compiler-verified aggregate module identity; `artifacts[].hash` is the SHA-256 of the corresponding committed artifact bytes. Lifecycle records bind the aggregate identity, while the per-artifact entry binds the physical tgz. |
...
- `resource.daemon_session_catalog` owns backend session catalog construction, backend-qualified `sessionCatalog` rows, `list-sessions` dispatch, and list-time idle fact publication through `src/server/daemon-session-catalog-runtime.ts`; it consumes `resource.backend_session` and `resource.session_idle_facts` only, and must not own client active/session, mirror store, transport subscriber, renderer, or UI truth.
...
- Android native `BackgroundService` is platform execution support for persistent-background mode. While the Activity is stopped it owns only the foreground-service notification and one process-level partial WakeLock so the existing control-plane and target-transport owners can continue their low-frequency heartbeats. It must not become a `resource.session_transport` owner, open/rebuild sockets, infer route health, request battery-optimization bypass, or keep body/video subscriptions active while app foreground truth is false.
...
- `resource.transport_subscriber` owns only physical `bodySubscribed`, send accounting, transport backpressure snapshot truth, and physical send eligibility. `resource.daemon_buffer_publisher` owns bounded pending-latest state and contiguous same-revision chunking for spans over the body-frame byte budget; it must not replace the source span with a shorter live-tail projection. Neither owner may store active tab, foreground, pane visibility, follow, reading, or viewport reasons.
...
- `resource.appsdk_compiler_identity` owns only the project-side AppSDK compiler identity preflight: the first executable selected by PATH, its exact bytes/version, and its binding to `.appsdk/project.json` plus `.appsdk/sdk.lock`. Package prebuild, CI, and Android release invoke record-graph verification through that same absolute path. The resource is control-only and has no direct relation to daemon, transport, mirror, renderer, UI, Active, or Protected truth.
