...
- `resource.remote_window_touch_action` owns client-side Direct Touch and Mouse Emulation classification. Tap emits one remote click; single-finger movement before hold emits realtime bounded pixel scroll at both 1x and zoomed scale; movement after a 250 ms hold emits reliable remote drag; stationary 500 ms hold emits one right click. Two-finger same-direction motion emits remote scroll at 1x and local pan while zoomed; anti-parallel distance change emits local pinch. Gesture mode latches, duration never invalidates release, pointer-cancel releases any remote down, and letterbox input maps to null. Mouse Emulation keeps remote pointer move/drag/wheel and requires explicit hand mode for local pan. The resource emits business actions only; delivery sequence/retry/ACK/health remains in typed input-delivery control resources and never enters action metadata.
