...
| `client.settings_update` | settings form, update route candidates, config import/export; typed `settings.update` slot render contract | settings projection, update check/install intent, plugin-provided AppUpdateSection projection | Settings/config/update UI projection and typed settings update UI slot contract | tab/session restore, forced Relay gate |
