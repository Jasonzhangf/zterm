...
describe('TerminalPage portrait session drawer', () => {
  ...
  it('does not enumerate disconnected stale relay daemon devices as empty drawer hosts', async () => {
    ...
    expect(screen.getByTestId('terminal-session-drawer-host-mac-studio').querySelector('span')?.style.background).toBe('var(--zterm-settings-success)');
  ...
  it('keeps daemon-catalog cwd when relay directory refresh has names without cwd', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('terminal-session-drawer-folder-button-/Volumes/extension/code/zterm')).toBeTruthy());
    expect(screen.getByTestId('terminal-session-drawer-folder-button-/Volumes/extension/code/zterm').textContent).toContain('zterm');
