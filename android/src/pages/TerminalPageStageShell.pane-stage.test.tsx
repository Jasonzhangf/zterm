...
describe('TerminalStageShell shared PaneStage integration (red baseline)', () => {
  ...
  it('does not give the session group center input ownership before runtime active catches up', async () => {
    const { TerminalStageShell } = await import('./TerminalPageStageShell');
    const s1 = {
      id: 's1',
      state: 'connected',
    ...
    const centerProps = terminalViewSpy.mock.calls.find((call) => call[0]?.sessionId === 's2')?.[0];
    expect(centerProps).toBeTruthy();
    expect(centerProps.active).toBe(false);
    expect(centerProps.onInput).toBeUndefined();
    expect(centerProps.onResize).toBeUndefined();
    expect(centerProps.onWidthModeChange).toBeUndefined();
  });
