...
export const REMOTE_WINDOW_TOUCH_DRAG_HOLD_MS = 250;
...
export function resolveRemoteWindowTouchSurfacePointRuntime(
  ...
  const normalizedX = (clientX - surfaceRect.left - contentRect.left) / Math.max(1, contentRect.width);
  const normalizedY = (clientY - surfaceRect.top - contentRect.top) / Math.max(1, contentRect.height);
...
export function resolveRemoteWindowTouchPointerMoveRuntime(options: {
  ...
  if (state.mode === 'actionPending' && state.pointerId === pointer.pointerId) {
    ...
    if (options.touchMode && pointer.pointerType === 'touch') {
      if (pointer.timeMs - state.startAtMs >= REMOTE_WINDOW_TOUCH_DRAG_HOLD_MS) {
        ...
        const moveEvent = buildRemoteWindowPointerInputEventRuntime({
          pointer: { ...pointer, buttons: 1 },
          geometry: options.geometry,
          phase: 'move',
          button: 'none',
          buttons: 1,
        ...
        }, [downEvent, moveEvent].filter((event): event is RemoteWindowInputEventPayload['event'] => Boolean(event)));
      }
      ...
      const { deltaX, deltaY } = resolveRemoteWindowPairScrollDeltaRuntime({
        rawDeltaX,
        rawDeltaY,
        surfaceRect: options.geometry.surfaceRect,
        sourceRect: options.geometry.sourceRect,
...
export function resolveRemoteWindowTouchPairPointerMoveRuntime(options: RemoteWindowTouchPairRuntimeOptions): RemoteWindowTouchPairRuntimeResult {
  ...
  if (state.mode === 'twoFingerPan') {
    return {
      ...
      localEffect: {
        kind: 'local-pan-move',
        pointerId: state.firstPointerId,
        deltaX: midpoint.clientX - state.startMidX,
        deltaY: midpoint.clientY - state.startMidY,
        moved: true,
      },
      consumed: true,
    };
  ...
  // Movement crossed the deadzone but has not committed to either gesture.
  return {
    nextState: { ...state, lastMidX: midpoint.clientX, lastMidY: midpoint.clientY },
    remoteEvents: [],
    localEffect: { kind: 'none' },
...
export function resolveRemoteWindowTouchPairPointerUpRuntime(options: {
  ...
  if (remainingPointer) {
    const remainingGesture: RemoteWindowTouchPointerState = remainingPointer.pointerType === 'touch'
      && options.remainingPointerMode === 'local-pan'
      ? {
          mode: 'localPan',
...
export function resolveRemoteWindowTouchPointerCancelRuntime(options: {
  ...
  if (state.mode === 'actionDrag' && statePointerId === pointer.pointerId) {
    ...
    return withRemoteEvents(idle, event ? [event] : []);
  }
  if (
    state.mode !== 'idle'
...
export function dispatchRemoteWindowTouchInputActionRuntime(options: {
  ...
  sendInput(action.sessionId, {
    ...
    ...(Number.isSafeInteger(action.layoutGeneration) ? { layoutGeneration: action.layoutGeneration } : {}),
    event: action.event,
  });
