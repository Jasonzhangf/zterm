...
export class AndroidConnectionServiceTransportSocket implements BridgeTransportSocket {
  ...
  get onmessage(): ((event: BridgeSocketMessageLike) => void) | null {
  ...
  private applySnapshot(snapshot: AndroidConnectionServiceSnapshot) {
    ...
    if (snapshot.state === 'resolving-target' || snapshot.state === 'connecting') {
      if (snapshot.generation && this.retiredGenerations.has(snapshot.generation)) {
        return;
      }
      if (this.readyGeneration) {
        if (!snapshot.generation || snapshot.generation === this.readyGeneration) {
          return;
        }
        ...
        && snapshot.generation
        && snapshot.generation !== this.pendingGeneration
      ) {
    ...
    if (snapshot.state === 'mux-ready' || snapshot.state === 'channels-ready' || snapshot.state === 'healthy') {
      if (
        (snapshot.generation != null && this.retiredGenerations.has(snapshot.generation))
        || (this.readyGeneration && snapshot.generation !== this.readyGeneration)
        || (this.pendingGeneration && snapshot.generation !== this.pendingGeneration)
      ) {
        return;
      }
      const muxReadyPayload = isRecord(snapshot.muxReadyPayload)
        ? snapshot.muxReadyPayload
        : this.muxReadyPayloadGeneration === snapshot.generation
          ? this.muxReadyPayload
          : null;
      if (!snapshot.generation || !muxReadyPayload) {
        return;
      }
      this.readyGeneration = snapshot.generation;
      this.pendingGeneration = null;
      this.muxReadyPayload = { ...muxReadyPayload };
      this.muxReadyPayloadGeneration = snapshot.generation;
    ...
    if (snapshot.state === 'backoff-reconnect') {
      if (snapshot.generation && this.readyGeneration && snapshot.generation !== this.readyGeneration) {
        return;
      }
      this.readyState = WebSocket.CLOSED;
      const retiredGeneration = snapshot.generation || this.readyGeneration || this.pendingGeneration;
      if (retiredGeneration) {
        this.retiredGenerations.add(retiredGeneration);
      }
      this.readyGeneration = null;
      this.pendingGeneration = null;
      this.muxReadyGeneration = null;
  ...
  private dispatchServerFrame(frame: AndroidConnectionServiceServerFrame) {
    if (this.disposed || frame.targetKey !== this.targetKey) {
      return;
    }
    ...
    if (
      ...
      || (!this.readyGeneration && this.pendingGeneration && frame.generation !== this.pendingGeneration)
    ) {
      return;
    }
    if (frame.type === 'mux-ready') {
      if (this.muxReadyGeneration === frame.generation) {
        return;
      }
  ...
  private scheduleOpenProjection() {
    if (this.openProjectionScheduled || this.disposed || !this.onopen) {
      return;
    }
    this.openProjectionScheduled = true;
    queueMicrotask(() => {
      this.openProjectionScheduled = false;
      if (this.disposed || this.readyState !== WebSocket.OPEN || !this.onopen) {
        return;
      }
      ...
      if (
        ...
        && this.muxReadyPayloadGeneration === generation
        && this.muxReadyPayload
      ) {
        this.muxReadyGeneration = generation;
        this.onmessage({
          data: JSON.stringify({ type: 'mux-ready', payload: this.muxReadyPayload }),
        });
      }
    });
  }
