...
export { createDefaultShortcutActions, isBuiltInShortcutAction };
...
export function normalizeShortcutActions(actions: DraftShortcutAction[]): TerminalShortcutAction[] {
  ...
  const defaultActions = createDefaultShortcutActions();
  const defaultActionsById = new Map(defaultActions.map((action) => [action.id, action]));
  actions.forEach((action) => {
    const builtInAction = defaultActionsById.get(action.id);
    if (builtInAction) {
      grouped.get(builtInAction.row)?.push({
        ...builtInAction,
        order: action.order,
      });
      return;
...
export function buildVisibleShortcutRowActions(
  ...
  consumedSequences = new Set<string>(),
) {
  const rowActions = shortcutActions.filter((action) => action.row === row);
  const customBySequence = new Set(
    rowActions
      .filter((action) => !isBuiltInShortcutAction(action.id))
      .map((action) => action.sequence),
  );
  const visibleActions: Array<{ id: string; label: string; sequence: string }> = [];
  rowActions.forEach((action) => {
    if (
      consumedSequences.has(action.sequence)
      || (isBuiltInShortcutAction(action.id) && customBySequence.has(action.sequence))
    ) {
      return;
    }
...
/**
 * Clamp the floating bubble position to the viewport (UI shell 几何纯函数)。
 */
export function clampFloatingBubblePosition(
  nextX: number,
  nextY: number,
  width: number,
  height: number,
