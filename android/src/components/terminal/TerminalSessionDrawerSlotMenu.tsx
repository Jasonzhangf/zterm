...
export function TerminalSessionDrawerSlotMenu({ sessionId, title, x, y, axis, onAssign, onClose }: Props) {
  ...
  return <div data-testid="terminal-session-drawer-slot-menu" style={{ position: 'absolute', left: `${Math.min(Math.max(12, x), 190)}px`, top: `${Math.max(72, y - 18)}px`, zIndex: 2, width: '160px', padding: '8px', borderRadius: '14px', border: '1px solid var(--zterm-panel-border)', background: 'var(--zterm-panel-bg)', boxShadow: '0 14px 30px var(--zterm-panel-shadow)', display: 'flex', flexDirection: 'column', gap: '6px' }}>
