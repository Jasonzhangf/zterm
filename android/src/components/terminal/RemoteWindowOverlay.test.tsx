...
describe('RemoteWindowOverlay', () => {
  ...
  it('refreshes a cached app target before starting and uses the unique fresh window identity', async () => {
    ...
    expect(requestTargets).toHaveBeenCalledTimes(4);
  });
  ...
  it('persists display orientation and quality controls, applies resize and start profile, and keeps sibling window switching', async () => {
    ...
    const requestTargets = vi.fn(async () => ({
      requestId: 'rw-controls-integration',
      targets: [childWindow, mainWindow],
    }));
    const resizeTargetWindow = vi.fn((_sessionId: string, _payload: Omit<RemoteWindowInputEventPayload, 'requestId'>) => 'resize-controls');
    ...
    await waitFor(() => {
      expect(Number.parseFloat(content.style.width || '0')).toBeCloseTo(1000, 1);
      expect(Number.parseFloat(content.style.height || '0')).toBeCloseTo(700, 1);
      expect(Number.parseFloat(content.style.left || '0')).toBeCloseTo(140, 1);
    ...
    await waitFor(() => expect(updateFocus).toHaveBeenCalledWith(
      'session-1',
      expect.any(String),
      childWindow,
      1,
  ...
  it('reads the daemon-owned active stream snapshot and does not poll the catalog', async () => {
    vi.useFakeTimers();
    const mediaStream = { id: 'media-stream-1' } as MediaStream;
    const target = makeTarget('app-main', 'WeChat', 'app-window');
    const requestTargets = vi.fn(async () => ({ requestId: 'rw-1', targets: [target] }));
    const startStream = vi.fn(async (_sessionId: string, _target: RemoteWindowStreamTargetManifest, streamId: string) => ({
      streamId,
      mediaStream,
    }));
  ...
  it('reopens the remote-window picker from the cached catalog without a blank loading panel', async () => {
    ...
    expect(screen.queryByTestId('remote-window-picker-loading')).toBeNull();
    expect(screen.getByTestId('remote-window-target-app-1')).toBeTruthy();
    // The reopen re-reads the daemon snapshot while the cached rows stay visible.
  ...
  it('tracks remote fill resize delivery per stream and only dedupes an accepted request', async () => {
    ...
    act(() => {
      remoteWindowMessageHandler?.({
        ...
        control: { version: 1, sequence: 'resize-1', accepted: false, retryable: false, duplicate: false, receivedAtMs: 1 },
        payload: { streamId: firstStreamId, targetId: 'app-1' },
      });
    });
    fireEvent.click(screen.getByTestId('remote-window-more-toggle'));
    fireEvent.click(screen.getByTestId('remote-window-fullscreen-display-toggle'));
    ...
    act(() => {
      remoteWindowMessageHandler?.({
        type: 'remote-window-input-ack',
        control: { version: 1, sequence: 'resize-2', accepted: true, retryable: false, duplicate: false, receivedAtMs: 2 },
        payload: {
  ...
  it('does not auto-open embedded catalog while app is backgrounded and reopens on foreground', async () => {
    ...
    view.rerender(renderOverlay(true));
    await screen.findByTestId('remote-window-target-app-catalog-loop');
    // Each foreground reopen re-reads the daemon-owned snapshot instead of
    // trusting a client-side projection cache.
    expect(requestTargets).toHaveBeenCalledTimes(2);
