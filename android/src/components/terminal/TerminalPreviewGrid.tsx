...
export function resolveSessionPreviewGridLayout(sessionCount: number, landscape: boolean) {
  const boundedCount = Math.max(1, Math.min(6, Math.trunc(sessionCount)));
  const columns = Math.min(boundedCount, landscape ? 3 : 2);
  return {
    columns,
    rows: Math.ceil(boundedCount / columns),
  };
}
...
export const TerminalPreviewGrid = memo(function TerminalPreviewGrid({
  ...
  const { columns, rows } = resolveSessionPreviewGridLayout(sessions.length, landscape);
            ...
            >
              ...
              >
                <span
                  ...
                  style={{
                    height: '24px', flexShrink: 0, display: 'grid',
                    gridTemplateColumns: '18px minmax(0, 1fr) 54px 20px', alignItems: 'center', gap: '4px',
                    padding: '0 4px 0 6px', background: tone.previewBackground, boxSizing: 'border-box',
                  }}
                >
                  <span style={{ color: tone.previewText, fontSize: '10px', fontWeight: 900 }}>{index + 1}</span>
                  <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: '10px', fontWeight: 800 }}>
                    {title}
                  </span>
                ...
                >
                  <TerminalView
                    ...
                    domInputOffscreen
                    focusNonce={0}
                    fontSize={Math.max(4, Math.min(6, fontSize - 4))}
                    rowHeight={`${Math.max(6, Math.min(9, fontSize - 1))}px`}
