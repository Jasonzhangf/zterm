...
export function useSessionProviderCoreAssemblies(
  ...
  const sessionInfraRuntime = useMemo(() => createSessionInfraFacadeRuntime({
    ...
    runtimeDebug,
  }), [options.bridgeSettings, options.dispatch, options.scheduleStatesRef, options.sessionStore, options.setScheduleStates, options.stateRef, options.terminalCacheLines, options.wsUrl]);
  ...
  const {
    ...
    stateRef: options.stateRef,
    terminalWidthMode: options.bridgeSettings.terminalWidthMode,
    runtimeDebug,
