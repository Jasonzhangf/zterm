...
function cursorIdentitySignature(cursor: TerminalBufferPayload['cursor']) {
  return `${scalarSignature(cursor?.rowIndex ?? null)}:${scalarSignature(cursor?.col ?? null)}:${scalarSignature(cursor?.visible ?? null)}`;
}

function frameMetadataSignature(payload: TerminalBufferPayload) {
  return [
    scalarSignature(payload.requestSentAt),
    scalarSignature(payload.availableStartIndex),
    scalarSignature(payload.availableEndIndex),
    scalarSignature(payload.cols),
    scalarSignature(payload.rows),
    scalarSignature(payload.cursorKeysApp),
    cursorIdentitySignature(payload.cursor),
  ].join(':');
}
...
function validateFrameChunkMetadata(payload: TerminalBufferPayload): ValidatedFrameChunk | null {
  ...
  return {
    frameKey: `${revision}:${frameStartIndex}:${frameEndIndex}:${generatedAt}:${frameChunkCount}:${frameMetadataSignature(payload)}`,
    revision,
    frameStartIndex,
    frameEndIndex,
    frameChunkIndex,
