...
describe('session buffer frame assembly', () => {
  ...
  it('includes full frame metadata in the frame identity before accepting another chunk', () => {
    const first = assembleBufferSyncFrameChunk(null, chunk({
      frameChunkIndex: 0,
      startIndex: 100,
      endIndex: 102,
    ...
    expect(first.kind).toBe('pending');
    const mismatchedMetadata = chunk({
      frameChunkIndex: 1,
      startIndex: 102,
      endIndex: 104,
    });
    mismatchedMetadata.requestSentAt = 9999;

    const second = assembleBufferSyncFrameChunk(
      first.state as BufferFrameAssemblyState,
      mismatchedMetadata,
    );
    ...
    if (second.kind !== 'rejected') {
      throw new Error(`expected rejected, got ${second.kind}`);
    }
    expect(second.error).toBe('interleaved-same-revision-frame');
    expect(second.state).toBeNull();
    expect(second.repairRange).toEqual({ startIndex: 100, endIndex: 104 });
    expect(second.repairRevision).toBe(11);
  });
