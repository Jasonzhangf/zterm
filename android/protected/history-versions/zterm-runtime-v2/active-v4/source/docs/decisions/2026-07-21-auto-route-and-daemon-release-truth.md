...
Automatic route selection is daemon-target scoped and does not ask the user to choose between Tailscale, UDP hole punch, or Relay/TURN. The canonical Auto order is:

1. `tailscale`
2. `rtc-direct` UDP hole punch
3. `rtc-relay` TURN/Relay
4. remaining direct IP candidates as explicit endpoint facts
